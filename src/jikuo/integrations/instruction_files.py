...
def common_instruction_lines() -> list[str]:
    return [
        ...
        "If you perform workspace writes in a turn, including file creation, edits, deletion, generated outputs, git staging, commits, or guarded project writes, run `python -B -m jikuo.agent_flow propose --event completion_review --project-root \"<absolute project root>\" --semantic-intent-ref \"anchor:<current turn_anchor anchor_id>\" --inherit-semantic-intent --format json` after verification and before the final response when a current turn anchor is available; otherwise run the same completion_review command without semantic inheritance and report degraded or missing semantic coverage.",
