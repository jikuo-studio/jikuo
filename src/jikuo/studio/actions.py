...
def registered_action_descriptors(
    *,
    ...
    return [
        ...
        action_descriptor(
            action_id="studio.artifact_assurance.review",
            domain="artifact_assurance",
            title="Review read/write assurance",
            write_mode="no-write",
            plan_surface="jikuo.studio.status.summaries.artifact_assurance",
            apply_surface=None,
            write_effect="none",
            approval_required=False,
            ...
            non_effects=[
                ...
                "does_not_replace_completion_review_evidence",
            ],
        ),
        action_descriptor(
        ...
        action_descriptor(
            ...
            domain="policy_management",
            title="Change active policy configuration",
            write_mode="no-write-plan",
            plan_surface="jikuo.propose_policy_evolution_plan",
            apply_surface="jikuo.apply_policy_evolution_write",
            write_effect="refine, deprecate, or supersede an active project policy only after guarded apply",
            approval_required=True,
        ...
        action_descriptor(
            action_id="studio.policy_final_response_gate.update",
            domain="policy_management",
            title="Update final-response gate setting",
            write_mode="guarded",
            plan_surface="/api/policy-management/status",
            apply_surface="/api/policy-management/final-response-gate/apply",
            write_effect="selected active policy YAML final_response_gate field only after confirmation and approval phrase",
            approval_required=True,
            source_ref="src/jikuo/policy_store.py",
            status="disabled" if policy_disabled else "available",
            disabled_reason=policy_disabled,
            non_effects=[
                ...
                "does_not_change_policy_trigger_conditions",
            ],
        ),
        action_descriptor(
            action_id="studio.policy_template.plan_publication",
            domain="policy_management",
            title="Make active policy reusable",
            write_mode="no-write-plan",
            plan_surface="jikuo.propose_policy_template_publication_plan",
            apply_surface="jikuo.apply_policy_template_publication",
            write_effect="one reusable package policy template only after guarded apply",
            approval_required=True,
            source_ref="src/jikuo/policy_templates.py",
            status="disabled" if policy_disabled else "available",
            disabled_reason=policy_disabled,
            non_effects=[
                "does_not_change_current_project_constraints",
                "does_not_write_without_confirmation",
            ],
        ),
        action_descriptor(
        ...
        action_descriptor(
            action_id="studio.policy_candidate.activate",
            domain="policy_management",
            title="Activate existing policy proposal",
            write_mode="guarded",
            plan_surface="/api/policy-management/candidate-activation/plan",
            apply_surface="/api/policy-management/candidate-activation/apply",
            write_effect="approved policy file, decision record, and manifest active ref only after guarded apply",
            approval_required=True,
            source_ref="src/jikuo/policy_store.py",
            status="disabled" if policy_disabled else "available",
            disabled_reason=policy_disabled,
            non_effects=[
                "does_not_rewrite_existing_proposal_snapshot",
                "does_not_execute_policy_actions",
            ],
        ),
        action_descriptor(
