...
Use this hook after the normal JIKUO first-run setup is understood:

1. Install JIKUO and confirm `jikuo.exe --help` and `jikuo-mcp.exe --help`
   work from the local environment.
2. Review activation settings with `jikuo configure status` or Studio.
3. Register the local stdio MCP server in Codex or the active desktop client.
4. Restart the client and verify it can call `jikuo.get_runtime_status_card`.
5. Review the client instruction file generated by `jikuo install --client codex`
   or explicitly choose to stay MCP-only.
6. Trust the project `.codex/` hook layer in Codex through the hook review UI,
   or use `/hooks` in CLI builds that expose hook review.
7. Submit a small test prompt and confirm a new JIKUO `conversation_turn` card
   appears under `.jikuo/runtime/history/`.

Expected visible hook output starts with:

```text
JIKUO mounted pre-turn ran before substantive model work.
```

If this line does not appear, treat the project as MCP plus instruction files,
not strict mounted execution. Review Codex hook trust, the configured Python
environment, and `.jikuo/runtime/last_card.md` before continuing.
