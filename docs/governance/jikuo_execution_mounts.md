...
- `JIKUO-MCP-01` Stage A acceptance requires the same policy runtime status to be visible through chat, `.jikuo/runtime/last_card.md`, and `jikuo show --last-card`, with `display_verification` as the recovery path if an Agent omits the card
- `JIKUO-MCP-01` Stage A SDK-free adapter boundary is implemented: `src/jikuo/integrations/mcp/adapter.py`, `schemas.py`, and `tests/mcp_adapter_tests.py` expose the 8 no-write / card / proposal tools without importing MCP SDK code; `server.py` and real-client smoke remain pending
- `JIKUO-MCP-01` official SDK availability was checked on 2026-05-15: the Python `mcp` package is not locally importable, so protocol `server.py` remains blocked on explicit dependency installation / declaration rather than a handwritten server
