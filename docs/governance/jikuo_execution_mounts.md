...
- Current program tracks:
  ...
  - Product surfaces: `JIKUO-STUDIO-01` is now the design anchor for a JIKUO-wide thin frontend / global console, not a policy-only frontend. The accepted sequence is global no-write view model first, panel/action registry second, local read-only console third, guarded configuration actions later; the `JIKUO-STUDIO-01A` landing plan now fixes the first read-model contract, data sources, status rules, action-preview descriptors, and tests. Studio must remain a projection and guarded control shell over canonical JIKUO state, not a second governance kernel. Per-client hook packs, Agent SDK / ADK adapters, OS notifications, desktop shell packaging, and hosted UI remain later expansion tracks.
