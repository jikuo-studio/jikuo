...
- Current program tracks:
  - MVP release track: `JIKUO-MVP-01` is the current fast-usable-version planning anchor. The MVP loop is configuration CRUD plus an independently checkable AI work receipt: users configure document/policy checklists, the host AI works, completion review gathers observable evidence, and Studio shows expected vs observed work plus gaps. The MVP must stay within the existing architecture: `agent_flow.py` orchestrates, `artifact_assurance.py` remains a pure comparison engine, git observation is a read-only completion-review adapter, runtime visibility stores the receipt projection, `global_status.py` exposes it, and Studio displays without inferring. Mount `MOUNT-MVP-WORK-RECEIPT-CONFIGURATION` before MVP implementation.
  ...
  - Strict mounted harness: Codex `UserPromptSubmit` hook proof is the current Codex host-boundary candidate, with Claude hook proof still tracked separately; MCP plus instruction files remain instruction-level, not strict mounted, until a host adapter proves otherwise. `JIKUO-HOSTADAPT-01` now defines the cross-client Host Adapter Turn Input/Result contract so Codex, Claude, Cursor, VS Code, and future wrappers/plugins can map their native hook events into the same portable JIKUO protocol. The Level 1 project-local Codex proof files now exist and local stdin smoke has passed, the local Level 2A input path can receive compact `host_semantic_intent` through CLI / MCP / Codex hook proof and merge it into `work_profile`, including local runtime-card projection for `requested_outcome`, `process_contract`, `execution_boundary`, and `response_contract`; a separate Codex GUI MCP router proof accepted that `route_user_request.host_semantic_intent` is visible and can be passed cooperatively by the host AI; an external Codex GUI MCP proposal-tool smoke later accepted direct `host_semantic_intent` re-call passthrough with `semantic_intent_evidence.status=ok` and no returned precondition; the Level 2B MCP Sampling probe can ask a Sampling-capable client for compact semantic intent during a tool call. A trusted Codex GUI run on 2026-05-21 accepted the narrower pre-turn `additionalContext` injection surface after timeout remediation, while a 2026-05-28 GUI probe exposed a visible nested-Python subprocess timeout; the project-local hook now defaults to in-process JIKUO invocation, fresh GUI smoke accepted the in-process pre-turn `additionalContext` path, a later GUI smoke confirmed the Host Adapter contract line in `additionalContext`, and a 2026-06-05 local hook smoke accepted isolated-surrogate prompt input without degraded output through surrogate-safe prompt hashing and display escaping. A refreshed Codex MCP config exposed the policy-distribution 19-tool surface including `jikuo.probe_sampling_semantic_intent`; the probe then returned Sampling unavailable via `sampling/createMessage`, so Codex client Sampling support remains unproven. The current source surface is 24 tools after policy-publication bridge tools plus the no-write policy-management status read model. Automatic hook-time semantic provider status, multi-intent semantic proof, and full lifecycle completion-review linkage remain pending. The hook proof must separately report mounted invocation status, semantic-intent status, multi-intent slice handling, and end-to-end GUI trigger flow: deterministic keyword routing is fallback/conflict evidence, not a substitute for host AI or dedicated semantic-classifier proof, MCP Sampling is optional classifier evidence rather than strict mounted proof, cooperative MCP-router/proposal-tool semantic transport is not automatic hook-time classification, multi-intent turns must preserve per-slice explanation plus aggregate policy scopes, and full strict-mounted lifecycle proof still needs visible pre-turn plus completion-review lifecycle cards.
    Current semantic-routing review conclusion: do not expand scopes or evaluator inputs to solve the missing-provider problem. The minimum hardening now implemented is a no-write `semantic_intent_precondition` / `precondition_unmet` tool response for selected governed editing / write-capable MCP entry points when `host_semantic_intent` is missing or invalid; those selected proposal tools accept `host_semantic_intent` directly on re-call; pure discussion fallback remains allowed.
  - Lifecycle / action-grammar infrastructure: `LIFECYCLE-01` is now the current primary foundation task, but the direction is minimum sustainable governance rather than a heavy fixed lifecycle. LIFECYCLE-01B implements the lighter record-only direction: once JIKUO is invoked for a user turn, runtime visibility should show the lifecycle nodes that actually produced cards (`conversation_turn`, `task_start`, `completion_review`, plus optional nodes), the observed/missing recommended node status, and card links. Customer-facing generated Markdown should end with an `Observed Lifecycle` footer that lists only `node_name: card_link` lines for the observed cards. LIFECYCLE-01C is the current design slice: define the minimum checkpoint contract around `pre-work` (`see -> classify -> trigger`), `governed-work` (`think -> act`), and `pre-final` (`see -> trigger -> report`) without adding a runner. It does not force missing nodes to run, does not prove GUI clients forced invocation, and does not make checkpoint design count as execution proof; host hook proof remains separate. Known architecture gap: MCP tools are callable surfaces and `agent_flow` is a node-local proposal runner, so no current layer guarantees `conversation_turn -> task_start -> completion_review` ordering. Architecture calibration: fixed lifecycle may be too rigid for AI work; `task_start` may remain an observed execution marker but should not become the source of policy applicability for intent-driven rules. Semantic routing should start from `user_intent -> policy_scope -> process_contract -> execution_boundary -> response_contract`, while `see / think / act / report` remains dynamic execution and evidence vocabulary. Contract-field effect must be proved through projection -> planning use -> evidence verification -> boundary enforcement/flagging; current evaluator consumption uses the implemented trigger event plus aggregate policy scopes, and consumes lifecycle only when a policy declares `lifecycle_events`. Track `INSIGHT-2026-05-21-lifecycle-sequencing-owner-gap` and `INSIGHT-2026-05-21-from-lifecycle-to-action-grammar` before designing a runner / harness sequencing owner.
  - Policy management: POLTRIG-01A / 01B / 02 / 03 are implemented. The two held user-authored candidates have been activated as `active_report_only` through reviewed scope-only policy plans and guarded policy writer output: first-principles / critical alignment is `conversation_turn` + `discussion`, and data-model drift alarm is `conversation_turn` + `editing`. Their policy-specific behavior stays in required actions, required evidence, and process-contract text rather than new policy-name-like scopes. Policy Management MVP closeout now documents source categories, official distribution flow, and active-policy maintenance outcomes; `policy_templates review-distribution` implements the no-write classification step, while `policy_distribution_review` agent-flow / MCP proposals expose the same review to GUI users through `policy_ref`, `source_policy`, or natural-language `policy_query` with ambiguity refusal. `policy_templates plan-publication` and guarded `publish-template`, plus `policy_template_publication_plan` / guarded `policy_template_publication` agent-flow and MCP surfaces, convert reusable outcomes into package-owned templates without activating user projects or mutating starter manifests; the first real optional package template has been published for `POLICY-jikuo-data-model-drift-alarm` and remains outside starter manifests. `starter_policies plan-publish-template` and guarded `publish-template --confirm-manifest-publication`, plus `starter_manifest_publication_plan` / guarded `starter_manifest_publication` agent-flow and MCP surfaces, add reviewed package template refs to starter manifests without initializing user projects.
  ...
  - Product surfaces: `JIKUO-STUDIO-01` is now the design anchor for a JIKUO-wide thin frontend / global console, not a policy-only frontend. `JIKUO-STUDIO-01A` implements the first no-write global status read model through `python -B -m jikuo studio status`, aggregating runtime, activation, configuration, integration, policy-management, registry, diagnostics, pending-decision, and action summaries without creating runtime cards or durable governance writes. `JIKUO-STUDIO-01B` implements no-write panel/action registries in `src/jikuo/studio/panels.py` and `src/jikuo/studio/actions.py`, so UI surfaces can render registered panels and guarded action affordances without owning governance logic. `JIKUO-STUDIO-01C` implements a local read-only HTTP console through `python -B -m jikuo studio serve`, backed by the same read models and no frontend-owned writes. `JIKUO-STUDIO-01D0` records the customer configuration action inventory and makes document mounting / main-document selection a first-class configuration domain before editable UI is implemented. `JIKUO-STUDIO-01D1` exposes document roles, main-document completion scope, active mount authority, and mount-set status through the same no-write Studio read model. `JIKUO-STUDIO-01D2` renders those document-mount facts as a dedicated read-only local web section. `JIKUO-STUDIO-01D3` maps stable customer-facing configuration language to internal refs so frontend copy does not drift with architecture changes. `JIKUO-STUDIO-01D4` documents the detailed Document Rules no-write plan and guarded apply contract, including validation, refusal, target-file, and frontend-flow boundaries, before implementation. `JIKUO-STUDIO-01D5` implements the plan-only backend through `python -B -m jikuo studio document-rules plan`. `JIKUO-STUDIO-01D6` exposes that backend through the local web console as a no-write preview form and `POST /api/document-rules/plan`. `JIKUO-STUDIO-01D7` separates editable configuration from governance guidance in the Studio read model and UI so structured configuration / registry can remain the long-term source of truth while human-readable governance documents become guidance or projection. `JIKUO-STUDIO-01D8` implements minimal guarded apply for reviewed Document Rules plans, writing only `.jikuo/project_context.yaml` after source-fingerprint verification, confirmation, and the exact approval phrase. `JIKUO-STUDIO-01D9` maps that backend approval contract to a natural web confirmation flow: users click `Approve and apply`, confirm the target and change count, and the frontend supplies the existing guarded evidence internally. `JIKUO-STUDIO-01D10` implements a no-write document/artifact assurance projection that compares configured required reads/writes with caller-supplied read evidence, planned writes, and actual writes, surfacing `required_not_planned`, `required_not_written`, `planned_not_written`, and `unplanned_written` without reading files, inspecting git, or proving model understanding by itself. `JIKUO-STUDIO-01D11` projects that comparison into runtime task cards, history cards, and `state_summary.artifact_assurance`, so the frontend can render recent task-level evidence instead of recomputing only the latest global snapshot. `JIKUO-STUDIO-01D12` corrects completion-check document semantics: configured completion scope is rendered as candidates/checklist items until applicability evidence makes an item required for the current slice. `JIKUO-STUDIO-01E1/01E2` implement a no-write project-file inventory (`GET /api/project-files`) and reusable local web batch selector for Document Rules. `selected_at_utc` is currently a per-batch UI cue, not canonical membership history; explicit membership timestamps remain a later `01E3` design. The next Studio sequence should render assurance projection in the frontend and decide canonical membership timestamp storage before heavier configuration editing. Studio must remain a projection and guarded control shell over canonical JIKUO state, not a second governance kernel. Per-client hook packs, Agent SDK / ADK adapters, OS notifications, desktop shell packaging, and hosted UI remain later expansion tracks.
...
- MCP implementation is active and accepted through the policy-management read
  model surface: current source discovery exposes 24 local stdio tools.
- this step is packaging-oriented: wrap stable `agent_flow.py`,
  `policy_store.py`, policy distribution, and read-model atoms without turning
  MCP itself into a policy evaluator.
...
- `JIKUO-CORE-09` implemented exact lifecycle trigger evaluation and required-action projection in `policy_store.py evaluate` and `agent_flow.py propose`; `POLTRIG-04` extends the evaluator so scope-only intent-driven policies can match by `work_profile.policy_scopes` across event surfaces.
...
- strict mounted behavior is not achieved by MCP plus instruction files alone; it requires a future pre-turn host adapter such as a Claude Code hook, Agent SDK wrapper, Studio/local proxy, Codex plugin, Cursor extension, or VS Code extension
  ...
  - `JIKUO-CODEX-PLUGIN-01` records the Codex hook / plugin feasibility task: Level 1 project-local `UserPromptSubmit` proof files exist, a 2026-05-21 trusted Codex GUI run accepted pre-turn `additionalContext` injection after timeout remediation, and a 2026-05-28 GUI probe exposed visible nested-Python subprocess timeout that is now remediated locally by default in-process JIKUO invocation with fresh GUI smoke accepted for the in-process pre-turn path, later contract-line projection, and a 2026-06-05 isolated-surrogate prompt/output smoke; Codex MCP availability itself is already accepted, while host-time semantic provider proof, multi-intent semantic proof, and full lifecycle strict-mounted acceptance remain pending
  - `JIKUO-HOSTADAPT-01` records the cross-client host adapter contract: normalized `jikuo.host_adapter.turn_input.v0` and `jikuo.host_adapter.turn_result.v0` records redact raw prompts, carry compact host semantic intent when supplied, and keep model calls, policy decisions, and durable writes outside the contract helper. Codex's feasible path is project-local hook for L1 mounted invocation, explicit semantic-intent transport for L2A, MCP Sampling for optional L2B classifier evidence, and a future wrapper/plugin for L3 host-time provider proof.
  - `JIKUO-AI-SEMROUTE-01` records the AI semantic routing MVP contract: host AI may pass compact `host_semantic_intent`, MCP Sampling may provide client-mediated classifier evidence after JIKUO is invoked, deterministic routing remains fallback / conflict evidence, MVP `policy_scopes` stay limited to `discussion`, `editing`, and `progress_summary`, and `intent_slices` / requested outcome / process / boundary / response fields remain card and evidence inputs rather than direct evaluator inputs. The first projection and consumption slice keeps `user_expression` short, filters unsupported scope labels, renders ordered intent slices in Work Profile cards, redacts raw `user_phrase` from trigger/router projections, proves locally that host semantic scopes can change scope-aware policy distribution through existing evaluator inputs, and accepts a cooperative Codex-host-AI explicit transport proof; `POLTRIG-04` now makes scope-only intent-driven policies scope-first so the declared trigger event is an evaluation-surface anchor rather than a lifecycle gate; the report-only `semantic_intent_classification_evidence` gate makes missing semantic classification visible for editing / progress work without calling a model provider or changing evaluator inputs; the MCP router server signatures and tool descriptions expose `host_semantic_intent`, and a separate Codex GUI proof confirmed that the router argument can be seen and passed with `semantic_intent_evidence.status=ok`. Selected governed editing / write-capable MCP entry points now opt into a no-write `semantic_intent_precondition` response when semantic evidence is missing or fallback-only. Automatic hook-time or Sampling semantic provider smoke remains pending and no strict host-time classification claim is made.
...
- `JIKUO-SELF-BOOTSTRAP-02` records the stable self-bootstrap execution strategy task: JIKUO development should consistently invoke route/config/task-start/completion-review tooling, bind or explicitly defer task sessions, surface runtime card links, and check main docs before commits; the active MCP/core-debug boundary policy is now backed by report-only evidence, where MCP task-start calls satisfy the MCP path and direct CLI/core calls must pass `--governance-path core_debug` to satisfy the explicit debug path
...
- `JIKUO-HOSTADAPT-01` records the shared host adapter contract. Its first concrete consumer is now the Codex project-local hook, which maps `UserPromptSubmit` input into `jikuo.host_adapter.turn_input.v0` and renders a `jikuo.host_adapter.turn_result.v0` summary before Codex-specific `additionalContext`; this remains adapter transport, not host-time semantic provider proof.
...
- policy evolution proposal/apply surfaces now separate replacement policy
  content from supersession lineage: replacement trigger semantics live in an
  already existing replacement policy, while `supersede_policy` links the old
  policy to that replacement without generating or editing replacement content
