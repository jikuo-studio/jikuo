...
- `JIKUO-INIT-01` records the first-use configuration review surface: `jikuo configure status|review`, `agent_flow propose --event configuration_review`, MCP `jikuo.get_configuration_status`, and MCP activation settings read / plan tools expose no-write setup review; mount `docs/work_orders/SPRINT_050_WO-PER-JIKUO-INIT-01_first_use_configuration_review.md` for future guarded configuration apply tools, Studio/dashboard setup views, or strict client-adapter onboarding work
