...
- Current program tracks:
  ...
  - Strict mounted harness: Codex `UserPromptSubmit` hook proof is the current Codex host-boundary candidate, with Claude hook proof still tracked separately; MCP plus instruction files remain instruction-level, not strict mounted, until a host adapter proves otherwise. The Level 1 project-local Codex proof files now exist and local stdin smoke has passed, and the local Level 2A input path can now receive compact `host_semantic_intent` through CLI / MCP / Codex hook proof and merge it into `work_profile`; these still only prove local adapter and input behavior until a trusted Codex GUI run shows pre-turn invocation, injected context, host-time semantic provider status, and completion-review card links. The hook proof must separately report mounted invocation status, semantic-intent status, multi-intent slice handling, and end-to-end GUI trigger flow: deterministic keyword routing is fallback/conflict evidence, not a substitute for host AI or dedicated semantic-classifier proof, multi-intent turns must preserve per-slice explanation plus aggregate policy scopes, and accepted proof needs visible pre-turn plus completion-review lifecycle cards.
...
- `JIKUO-CODEX-PLUGIN-01` records the Codex hook / plugin feasibility task: Level 1 project-local `UserPromptSubmit` proof files exist, but a real trusted Codex GUI run must still prove pre-turn execution before claiming strict mounted behavior; Codex MCP availability itself is already accepted, so the remaining question is host-level enforcement
