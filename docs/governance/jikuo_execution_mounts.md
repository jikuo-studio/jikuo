...
- Current program tracks:
  ...
  - Policy management: POLTRIG-01A / 01B / 02 / 03 are implemented. The two held user-authored candidates have been activated as `active_report_only` through reviewed scope-only policy plans and guarded policy writer output: first-principles / critical alignment is `conversation_turn` + `discussion`, and data-model drift alarm is `conversation_turn` + `editing`. Their policy-specific behavior stays in required actions, required evidence, and process-contract text rather than new policy-name-like scopes. Next policy-management work is official distribution boundary design: dogfood-only, official starter, optional template, or deferred.
