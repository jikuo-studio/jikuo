...
- `JIKUO-LIVE-21`: governed slice task-session resolution, implemented on 2026-05-17 and pending review; broadens the task-session binding policy from code-only slices to all `task_start` events and surfaces unresolved lifecycle carrier state as `needs_user_decision`
- `JIKUO-LIVE-16`: policy runtime card priority, implemented on 2026-05-15 and ready for user review; `chat_ready_markdown` and runtime cards render `policy_runtime_status` before task-session / lifecycle cards, and proposal JSON exposes `display.card_priority_order`
- `JIKUO-LIVE-17`: task-session index stale hint, implemented on 2026-05-15 and ready for user review; `jikuo show` reports whether `latest_task_session_refs` is current or stale and surfaces dry-run / guarded refresh commands without writing project state
- `JIKUO-LIVE-18`: previous/latest todo snapshot posture, implemented on 2026-05-15 and ready for user review; removes the fake same-file `previous_todo_map` binding, records snapshot rotation as a deferred insight, and guards against treating latest-vs-previous comparison as available in v0
...
- `JIKUO-INTG-03`: strict mounted harness adapter contract, accepted on 2026-05-16; defines the host-adapter layer required to make mounted mode strict instead of prompt-only, and stops further work at client proof before choosing the first adapter implementation
- `JIKUO-MCP-01`: MCP wrapper MVP body implemented and release-smoked; Stage A accepted as user-verified, Stage B1 implemented after explicit user approval, Stage B2 implemented plus externally smoke-accepted on 2026-05-16, and Stage B3 implemented plus externally smoke-accepted on 2026-05-16; post-MVP configuration expansion exposes `jikuo.get_configuration_status`, `jikuo.get_activation_settings`, `jikuo.plan_activation_settings_update`, guarded `jikuo.apply_activation_settings_update`, and no-write router tools `jikuo.route_user_request` / `jikuo.propose_policy_suggestions`; `src/jikuo/integrations/mcp/adapter.py`, `schemas.py`, and `server.py` now expose 17 tools
...
- `JIKUO-POLICY-CATALOG-01`: self-bootstrap policy promotion review, accepted for code-level hardening on 2026-05-16; starter distribution now treats JIKUO's own `.jikuo/policies` as dogfood state, not an upstream source, and official starter packs may reference only package-owned policy templates
...
- `POLICY-jikuo-progress-summary-business-meaning`: active report-only self-bootstrap policy approved on 2026-05-16; completion summaries and todo lists should include business / product meaning so non-expert users can judge the work without reading implementation-only language
...
- `JIKUO-INIT-01`: first-use configuration review, implemented as a no-write core aggregator, agent-flow card, and read-only MCP configuration-status tool; configuration update tools and strict client adapters remain planned
- `JIKUO-INIT-02`: reviewed configuration onboarding, implemented on 2026-05-16; activation settings now distinguish implicit defaults, unreviewed legacy files, user-reviewed defaults, and user-configured values, and first active use routes unreviewed settings to configuration review before task execution
- `JIKUO-CODEX-PLUGIN-01`: Codex plugin pre-turn harness review, planned on 2026-05-16; verifies whether Codex plugin surfaces can support strict pre-turn JIKUO routing or should remain instruction / MCP setup helpers only
- `JIKUO-LIVE-20`: policy dead-zone detection, LIVE-20A implemented on 2026-05-17; detects and explains active-policy evaluations with zero triggered policies so users do not mistake governance no-ops for healthy coverage
- `JIKUO-DATA-01`: structured execution event ledger and analytics contract, planned on 2026-05-17; defines the future local execution-data layer for dashboard, BI, support bundles, and audit projections so those surfaces do not scrape Markdown cards or raw chat transcripts
- `JIKUO-DOCREG-01`: layered document registry, planned on 2026-05-17; acts as the transitional source of truth for the current DOCREG task sequence until `docs/registry/registry_index.yaml` and first shards exist
- `JIKUO-SELF-BOOTSTRAP-02`: stable self-bootstrap execution strategy, planned on 2026-05-17; defines how JIKUO development itself should consistently invoke route/config/task-start/completion-review tooling before relying on assistant memory
- `JIKUO-SELF-BOOTSTRAP-03`: harness workspace boundary spike, planned on 2026-05-17; tests whether self-bootstrap should separate parent harness state from child source repositories before any directory migration
- `JIKUO-MIGRATION-01`: NarrativeSystem resource-pool handoff recorded on 2026-05-16; standalone JIKUO no longer has active runtime / contract dependencies on NarrativeSystem `docs/jikuo/` or `tools/jikuo/`, and remaining old-path mentions are historical provenance, fixture-only, or protective tests
...
| `self_bootstrap_policy_promotion_review` | A user installs or updates JIKUO and must never have JIKUO's dogfood policies silently copied over, merged into, or used to overwrite the user's own project policies | maintainer reviews JIKUO self-bootstrap policies -> classifies each as dogfood-only / official starter / optional template / deferred -> promoted policies become package templates -> starter pack manifests reference only `pkg://jikuo/policy_templates/...` -> guarded starter/template activation appends reviewed policies to the target project without overwriting user policies | `CAP-STARTER-POLICY-DISTRIBUTION-BOUNDARY-01`; `CAP-STARTER-POLICY-PACK-INIT-01`; `CAP-POLICY-TEMPLATE-EXTRACT-01`; `CAP-POLICY-TEMPLATE-ACTIVATE-01`; `CAP-POLICY-STORE-APPEND-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-POLICY-CATALOG-01_self_bootstrap_policy_promotion_review.md` |
| `starter_policy_provenance_backfill` | A JIKUO project initializes official starter policies and needs those approved policies to carry auditable package provenance before any MCP exposure | load official starter pack -> copy template policy into starter record -> attach `verified_jikuo_official` provenance -> expose provenance in plan summary -> guarded init writes approved policy with provenance | `CAP-STARTER-POLICY-PROVENANCE-01`; `CAP-STARTER-POLICY-PACK-INIT-01`; `CAP-TRUST-PRIVACY-PROVENANCE-BASELINE-01`; `CAP-MCP-PREIMPLEMENTATION-API-NEUTRALITY-REVIEW-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-LIVE-19_starter_policy_provenance_backfill.md` |
...
| `policy_runtime_first_screen_priority` | A desktop Agent user needs policy runtime status to be the first visible governance card, not buried behind lifecycle cards | agent invokes `agent_flow.py propose` -> proposal attaches display directives with `policy_runtime_status` first -> chat-ready markdown renders prioritized cards before detailed trace sections -> runtime snapshot stores the same prioritized card order | `CAP-POLICY-RUNTIME-CARD-PRIORITY-01`; `CAP-POLICY-RUNTIME-STATUS-CARD-01`; `CAP-DESKTOP-CHAT-HARNESS-SURFACE-01`; `CAP-RUNTIME-VISIBILITY-CHANNEL-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-LIVE-16_policy_runtime_card_priority.md` |
...
| `conversation_level_policy_suggestion` | A JIKUO user repeatedly expresses a need during ordinary conversation, and JIKUO should propose a reviewable policy candidate without waiting for the exact phrase "make this a policy" | user turn arrives -> trigger-mode router classifies semantic vs mounted harness coverage -> conversation-turn router checks repeated needs / corrections / preferences -> policy-suggestion review produces compact evidence and candidate cards -> user approves, revises, defers, records as insight, or ignores -> guarded policy write remains separate | `CAP-JIKUO-TRIGGER-MODE-01`; `CAP-CONVERSATION-TURN-ROUTER-01`; `CAP-PROACTIVE-POLICY-SUGGESTION-REVIEW-01`; `CAP-PROGRESS-SUMMARY-BUSINESS-MEANING-POLICY-01`; `CAP-MCP-AGENT-FLOW-WRAPPER-01`; `CAP-AGENTS-SDK-ADAPTER-EXPLORATION-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-ROUTER-01_trigger_mode_and_conversation_turn_router.md`; `docs/insights/INSIGHT-2026-05-16-proactive-policy-suggestion-metapolicy.md` |
...
| `client_onboarding_settings` | A user enables JIKUO in a desktop Agent client and needs every activation setting to be explicit rather than hidden in defaults | settings plan previews project defaults -> guarded settings apply writes `.jikuo/activation_settings.yaml` after approval -> install plan selects client and reads the project default unless overridden -> managed instruction block records activation settings -> client follows MCP / instruction contract -> strict mounted execution remains adapter-gated | `CAP-CLIENT-ONBOARDING-SETTINGS-01`; `CAP-JIKUO-TRIGGER-MODE-01`; `CAP-UNIVERSAL-INSTRUCTION-FILE-01`; `CAP-CONVERSATION-TURN-ROUTER-01`; `CAP-MCP-AGENT-FLOW-WRAPPER-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-INTG-02_client_onboarding_settings_and_trigger_mode.md`; `docs/integrations/mcp_client_configuration_examples.md` |
| `strict_mounted_adapter_contract` | A user chooses mounted mode and expects JIKUO to run before every turn instead of depending on agent memory | activation settings declare mounted mode -> host adapter reads `.jikuo/activation_settings.yaml` -> adapter calls JIKUO before each user turn -> card markdown or runtime links are surfaced -> guarded writes remain separate approval paths -> client proof decides the first reliable adapter host | `CAP-STRICT-MOUNTED-HARNESS-ADAPTER-CONTRACT-01`; `CAP-JIKUO-TRIGGER-MODE-01`; `CAP-CONVERSATION-TURN-ROUTER-01`; `CAP-MCP-CONVERSATION-ROUTER-SURFACE-01`; `CAP-CLIENT-ONBOARDING-SETTINGS-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-INTG-03_strict_mounted_harness_adapter_contract.md` |
| `first_use_configuration_review` | A user first enables JIKUO and needs to understand current setup state and business meaning before governed work continues | user asks to initialize / configure JIKUO -> no-write configuration review aggregates activation, instructions, runtime, MCP, project context, starter policies, and guarded writes -> agent-flow card explains status and next actions -> MCP read / plan tools expose the same setup state inside clients -> guarded MCP apply writes reviewed activation settings after approval -> later Studio surfaces reuse the same review object | `CAP-CONFIGURATION-REVIEW-01`; `CAP-MCP-ACTIVATION-SETTINGS-READ-PLAN-01`; `CAP-MCP-ACTIVATION-SETTINGS-APPLY-01`; `CAP-CLIENT-ONBOARDING-SETTINGS-01`; `CAP-RUNTIME-SHOW-CLI-01`; `CAP-MCP-AGENT-FLOW-WRAPPER-01`; `CAP-CONVERSATION-TURN-ROUTER-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-INIT-01_first_use_configuration_review.md` |
| `reviewed_configuration_onboarding` | A user first actively triggers JIKUO and must not have implicit defaults mistaken for completed setup | activation settings status detects missing / legacy-unreviewed provenance -> configuration review reports required user decisions -> conversation router injects `configuration_review` before task work when no explicit mode override exists -> guarded activation apply records either `user_reviewed_default` or `user_configured` -> later route/status calls treat the value as reviewed project configuration | `CAP-CONFIGURATION-SOURCE-PROVENANCE-01`; `CAP-CLIENT-ONBOARDING-SETTINGS-01`; `CAP-CONFIGURATION-REVIEW-01`; `CAP-CONVERSATION-TURN-ROUTER-01`; `CAP-MCP-ACTIVATION-SETTINGS-READ-PLAN-01`; `CAP-MCP-ACTIVATION-SETTINGS-APPLY-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-INIT-02_reviewed_configuration_onboarding.md` |
| `codex_plugin_pre_turn_harness_review` | A Codex user wants JIKUO to be more than an optional MCP tool and needs proof whether a Codex plugin can force every-turn routing | inspect Codex plugin surfaces -> classify skills / instructions / MCP setup vs pre-turn hook capability -> test whether `jikuo.route_user_request` can run before each user turn -> record proof and fallback posture -> only then decide whether to implement a Codex strict mounted adapter or an instruction-only helper plugin | `CAP-CODEX-PLUGIN-HARNESS-REVIEW-01`; `CAP-STRICT-MOUNTED-HARNESS-ADAPTER-CONTRACT-01`; `CAP-CONVERSATION-TURN-ROUTER-01`; `CAP-MCP-CONVERSATION-ROUTER-SURFACE-01`; `CAP-UNIVERSAL-INSTRUCTION-FILE-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-CODEX-PLUGIN-01_codex_plugin_pre_turn_harness_review.md` |
| `claude_hook_strict_adapter_review` | A Claude user wants mounted mode to call JIKUO before every turn rather than depending on `CLAUDE.md` instruction compliance | inspect Claude hook-capable host surfaces -> classify MCP / instruction-only support vs pre-turn hook support -> test whether the hook can call `jikuo.route_user_request` before the model turn -> verify card / runtime-link surfacing and visible failure behavior -> record proof and fallback posture -> only then decide whether to implement a Claude strict mounted adapter | `CAP-CLAUDE-HOOK-HARNESS-REVIEW-01`; `CAP-STRICT-MOUNTED-HARNESS-ADAPTER-CONTRACT-01`; `CAP-CONVERSATION-TURN-ROUTER-01`; `CAP-MCP-CONVERSATION-ROUTER-SURFACE-01`; `CAP-UNIVERSAL-INSTRUCTION-FILE-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-CLAUDE-HOOK-01_claude_hook_strict_adapter_review.md` |
| `policy_dead_zone_detection` | A user sees JIKUO runtime cards but every active-policy evaluation reports zero triggered policies, creating false confidence that governance is working | inspect recent runtime history -> classify zero-trigger cards by event and context completeness -> warn when repeated no-trigger behavior suggests wrong invocation, missing task context, missing activation, or policy coverage gaps -> point user to route/task-start context, starter policy expansion, or client adapter fixes | `CAP-POLICY-DEAD-ZONE-DETECTION-01`; `CAP-RUNTIME-VISIBILITY-CHANNEL-01`; `CAP-POLICY-RUNTIME-STATUS-CARD-01`; `CAP-CONVERSATION-TURN-ROUTER-01`; `CAP-MCP-CONVERSATION-ROUTER-SURFACE-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-LIVE-20_policy_dead_zone_detection.md` |
| `structured_execution_event_ledger` | A user or maintainer needs future dashboard, BI, support, and audit views to inspect JIKUO execution without parsing Markdown cards or raw chat transcripts | inspect current proposal/runtime/task-session/policy/MCP structures -> classify source state vs projection vs candidate event facts -> define schema-versioned local event ledger -> define correlation / privacy / projection rules -> stage implementation before dashboard work | `CAP-STRUCTURED-EXECUTION-EVENT-LEDGER-01`; `CAP-RUNTIME-VISIBILITY-CHANNEL-01`; `CAP-POLICY-RUNTIME-STATUS-CARD-01`; `CAP-POLICY-DEAD-ZONE-DETECTION-01`; `CAP-MCP-AGENT-FLOW-WRAPPER-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-DATA-01_structured_execution_event_ledger_and_analytics_contract.md` |
| `document_registry_transition` | A JIKUO maintainer needs the task map, capability registry, scenario chains, and mount rules to become machine-readable without losing the current roadmap during migration | create transitional DOCREG work order -> register it in project context and docs entry points -> define layered registry authority -> implement registry shards and tests -> migrate future task ordering into `docs/registry/work_orders.yaml` | `CAP-DOCUMENT-REGISTRY-TRANSITION-01`; `CAP-TASKMAP-INSIGHT-FOLLOWUP-EVIDENCE-01`; `CAP-STRUCTURED-EXECUTION-EVENT-LEDGER-01`; `CAP-SELF-BOOTSTRAP-HARNESS-WORKSPACE-BOUNDARY-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-DOCREG-01_layered_document_registry.md` |
| `stable_self_bootstrap_execution` | A JIKUO maintainer needs the assistant to use JIKUO itself consistently during JIKUO development, rather than only following policy intent from memory | start governed slice -> call JIKUO route/config/task-start path -> bind or defer task-session visibly -> implement work -> run tests -> run completion/main-doc review -> surface latest runtime card link -> commit only after review evidence is visible | `CAP-STABLE-SELF-BOOTSTRAP-EXECUTION-01`; `CAP-CONVERSATION-TURN-ROUTER-01`; `CAP-CONFIGURATION-REVIEW-01`; `CAP-TASK-SESSION-BINDING-EVIDENCE-01`; `CAP-MAIN-DOC-MOUNT-MAINTENANCE-01`; `CAP-RUNTIME-VISIBILITY-CHANNEL-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-SELF-BOOTSTRAP-02_stable_self_bootstrap_execution_strategy.md` |
| `self_bootstrap_harness_workspace_boundary` | A JIKUO maintainer needs self-bootstrap proof to model JIKUO as a harness around a governed source repository, not just a tool inside its own source root | create temporary parent workspace -> place `.jikuo/`, activation settings, runtime cards, and client instructions at harness root -> mount source repository as a child target -> test route/config/task-start and MCP calls against target source -> record project_root vs target_project_root implications -> recommend keep / adopt / generalize / defer | `CAP-SELF-BOOTSTRAP-HARNESS-WORKSPACE-BOUNDARY-01`; `CAP-STABLE-SELF-BOOTSTRAP-EXECUTION-01`; `CAP-STRICT-MOUNTED-HARNESS-ADAPTER-CONTRACT-01`; `CAP-CONVERSATION-TURN-ROUTER-01`; `CAP-MCP-CONVERSATION-ROUTER-SURFACE-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-SELF-BOOTSTRAP-03_harness_workspace_boundary_spike.md`; `docs/insights/INSIGHT-2026-05-17-self-bootstrap-harness-workspace-boundary.md` |
| `narrativesystem_resource_pool_handoff` | A JIKUO maintainer needs to archive old incubating `docs/jikuo` and `tools/jikuo` files from NarrativeSystem without leaving hidden active dependencies in the standalone package | scan standalone repo for old NarrativeSystem paths -> classify references as active runtime / active contract / historical provenance / fixture-only / no-action-needed -> migrate active JIKUO refs to package/module paths -> write handoff markdown and YAML manifest -> leave NarrativeSystem untouched until a separate write plan | `CAP-PROJECT-CONTEXT-BINDING-01`; `CAP-MAIN-DOC-MOUNT-MAINTENANCE-01` | `docs/migration/NARRATIVESYSTEM_RESOURCE_POOL_HANDOFF.md`; `docs/migration/narrativesystem_resource_pool_handoff.yaml` |
...
| `mcp_preimplementation_api_neutrality_review` | A JIKUO maintainer needs to verify the first MCP wrapper will consume core APIs without becoming kernel code | bind governed task-session -> inspect `agent_flow.py`, `policy_store.py`, `runtime_visibility.py`, policy templates, and starter policies -> record pass / caution / blocker findings -> freeze allowed core call surfaces before MCP implementation | `CAP-MCP-PREIMPLEMENTATION-API-NEUTRALITY-REVIEW-01`; `CAP-INTEGRATION-NEUTRALITY-CONTRACT-01`; `CAP-MCP-AGENT-FLOW-WRAPPER-01`; `CAP-RUNTIME-VISIBILITY-CHANNEL-01`; `CAP-TASK-SESSION-BINDING-EVIDENCE-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-ARCH-03_mcp_pre_implementation_api_neutrality_review.md`; `.jikuo/task_sessions/task_20260514T170932Z_mcp_pre_implementation_api_neutrality_review_82553d04.yaml` |
...
| `self_bootstrap_task_session_binding` | A JIKUO maintainer starts a governed development slice and needs a durable lifecycle carrier before evidence / completion / handoff can be trusted | task-start proposal -> task-session resolution evidence -> policy evaluator matches binding evidence for any governed task_start -> user either binds existing session, approves guarded creation, or explicitly defers -> later completion can target the session only when one exists | `CAP-TASK-SESSION-BINDING-EVIDENCE-01`; `CAP-TASK-SESSION-RESOLUTION-01`; `CAP-TASK-START-DRYRUN-01`; `CAP-AGENT-FLOW-APPLY-TASK-SESSION-START-01`; `CAP-TASK-START-WRITE-01`; `CAP-POLICY-RUNTIME-STATUS-CARD-01`; `CAP-RUNTIME-VISIBILITY-CHANNEL-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-LIVE-15_self_bootstrap_task_session_binding.md`; `docs/work_orders/SPRINT_050_WO-PER-JIKUO-LIVE-21_governed_slice_task_session_resolution.md`; `.jikuo/task_sessions/task_20260514T130539Z_jikuo_live_15_self_bootstrap_task_session_bindin_aa0dd272.yaml` |
| `task_session_index_stale_hint` | A JIKUO maintainer needs to know when task-session files exist but `.jikuo/project_state.yaml latest_task_session_refs` is stale | user runs `jikuo show` -> runtime visibility loads state summary -> index refresh planner compares current refs with discovered task-session files -> output shows stale/current status plus dry-run and guarded refresh commands | `CAP-RUNTIME-SHOW-CLI-01`; `CAP-TASK-INDEX-REFRESH-01`; `CAP-TASK-SESSION-INDEX-STALE-HINT-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-LIVE-17_task_session_index_stale_hint.md` |
| `previous_latest_todo_snapshot_posture` | A JIKUO maintainer needs project context to stop pretending a previous todo snapshot exists before snapshot rotation is implemented | review project context -> set `previous_todo_map.path` to `null` -> record snapshot rotation as deferred insight -> update taskmap / mounts -> regression test guards against same-file fallback | `CAP-PROJECT-CONTEXT-BINDING-01`; `CAP-TODO-SNAPSHOT-POSTURE-01`; `CAP-MAIN-DOC-MOUNT-MAINTENANCE-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-LIVE-18_previous_latest_todo_snapshot_posture.md`; `docs/insights/INSIGHT-2026-05-15-todo-snapshot-rotation.md` |
| `mcp_response_privacy_classification` | A JIKUO maintainer needs MCP responses to expose useful governance cards without leaking local-only or sensitive fields | identify MCP response field kinds -> classify as `return` / `local_only` / `redact_required` / `redact_optional` -> update SEC-01 -> update MCP startup checklist and tests -> keep implementation blocked until accepted | `CAP-MCP-RESPONSE-PRIVACY-CLASSIFICATION-01`; `CAP-TRUST-PRIVACY-PROVENANCE-BASELINE-01`; `CAP-MCP-PREIMPLEMENTATION-API-NEUTRALITY-REVIEW-01`; `CAP-MCP-AGENT-FLOW-WRAPPER-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-SEC-02_mcp_response_privacy_classification_baseline.md` |
...
- self-bootstrap task-session records exist under `.jikuo/task_sessions/`; `.jikuo/project_state.yaml latest_task_session_refs` refresh remains a separate guarded action with stale/current status surfaced by `jikuo show`
- no hook
- no CI gate
- no task-stop gate
- no frontend UI
...
> **DOCREG-01B1 transition note**: This section is frozen as of 2026-05-17.
> Capability metadata changes now go to `docs/registry/capabilities.yaml`.
> This Markdown section remains as a human projection during the migration
> window and is expected to be regenerated or replaced by DOCREG-01C.

> **历史注释**：本节最初用于承载机括原子能力注册表，避免过早拆出独立 registry。DOCREG-01A/B1 已完成 `docs/registry/capabilities.yaml` 迁移，本节现在只保留为冻结的人类投影，等待 DOCREG-01C 再生成或替换。
...
| `CAP-MAIN-DOC-MOUNT-MAINTENANCE-01` | Standalone main document mount maintenance | implemented governance mount | `README.md`; `.jikuo/project_context.yaml`; `docs/README.md`; `docs/governance/jikuo_execution_mounts.md`; `docs/governance/jikuo_productization_task_map.md`; `POLICY-jikuo-main-doc-mount-maintenance` | keeps active document paths and slice-completion check scope visible before each JIKUO development slice closes | docs / project context only | report-only completion evidence required |
...
| `CAP-JIKUO-TRIGGER-MODE-01` | JIKUO trigger-mode contract | implemented core / settings / instruction contract | `python -B -m jikuo.agent_flow propose --event conversation_turn --user-phrase ... --trigger-mode semantic|mounted`; `jikuo settings plan|apply`; `jikuo install --trigger-mode ask|semantic|mounted` | distinguishes natural-language semantic triggering from mounted harness triggering, so users can choose lightweight invocation or strict pre-turn harness execution and see that choice in project settings plus client onboarding instructions | `.jikuo/activation_settings.yaml` only through guarded settings apply; instruction files only through guarded install | strict mounted execution still requires a host adapter; no client should silently default to mounted without user choice |
| `CAP-CONVERSATION-TURN-ROUTER-01` | Conversation-turn router | implemented core no-write router plus MCP surface | `python -B -m jikuo.agent_flow propose --event conversation_turn --user-phrase ... --trigger-mode semantic|mounted`; `jikuo.route_user_request` | classifies ordinary user turns into JIKUO obligations, including task start, completion, policy suggestion review, insight / follow-up routing, clarification, semantic no-op, and mounted idle ticks, instead of relying only on task-specific events | none beyond runtime visibility projection | strict mounted adapters remain future work |
| `CAP-PROACTIVE-POLICY-SUGGESTION-REVIEW-01` | Proactive policy-suggestion review | implemented core no-write review plus MCP surface | `python -B -m jikuo.agent_flow propose --event conversation_turn --user-phrase ...`; `jikuo.propose_policy_suggestions` | produces `proactive_policy_suggestion_review_evidence` and reviewable candidate cards from repeated user needs / corrections / preferences without raw transcript capture | none beyond runtime visibility projection | policy activation remains guarded and requires explicit approval |
| `CAP-PROGRESS-SUMMARY-BUSINESS-MEANING-POLICY-01` | Progress-summary business-meaning policy | implemented active report-only policy | `.jikuo/policies/approved/POLICY-jikuo-progress-summary-business-meaning.yaml`; `python -B -m jikuo.policy_store evaluate --event completion_review ...` | makes progress, todo, taskmap, and acceptance summaries understandable by requiring the business / product meaning of major items, not only implementation mechanics | policy-store proposal, approved policy, decision record, and manifest update | report-only evidence required on completion review |
...
| `CAP-POLICY-RUNTIME-CARD-PRIORITY-01` | Policy runtime first-screen card priority | implemented by `JIKUO-LIVE-16` | `src/jikuo/agent_flow.py`; `python -B -m jikuo.agent_flow propose ...` | returns protocol-neutral display directives and renders `policy_runtime_status` before lifecycle / task / write-plan cards in chat-ready and runtime-card Markdown | none | structured `cards[]` generation order remains stable; display order is deterministic |
...
| `CAP-RUNTIME-SHOW-CLI-01` | Runtime visibility CLI | implemented read-only CLI | `jikuo show`; `jikuo show --last-card`; `python -B -m jikuo show` | lets users inspect current runtime status and project activation settings without relying on Agent chat behavior | none | read-only |
...
| `CAP-TASK-SESSION-INDEX-STALE-HINT-01` | Task-session index stale hint | implemented read-only CLI projection | `jikuo show`; `src/jikuo/runtime_visibility.py`; `src/jikuo/task_session.py build_index_refresh_plan` | shows whether `.jikuo/project_state.yaml latest_task_session_refs` is current relative to discovered `.jikuo/task_sessions/*.yaml` files and surfaces dry-run / guarded refresh commands | none | `jikuo show` must not mutate project state |
| `CAP-TODO-SNAPSHOT-POSTURE-01` | Previous/latest todo snapshot posture | implemented context/documentation guard | `.jikuo/project_context.yaml`; `docs/insights/INSIGHT-2026-05-15-todo-snapshot-rotation.md`; `tests/policy_templates_tests.py` | disables fake previous-todo binding in v0 and records guarded snapshot rotation as the future capability required for real latest-vs-previous comparison | none | no policy should treat previous/latest comparison as available while `previous_todo_map.path` is null |
| `CAP-STARTER-POLICY-PROVENANCE-01` | Starter policy provenance backfill | implemented metadata guard | `src/jikuo/starter_policies.py`; `tests/starter_policies_tests.py`; `docs/work_orders/SPRINT_050_WO-PER-JIKUO-LIVE-19_starter_policy_provenance_backfill.md` | attaches `verified_jikuo_official` provenance to official starter policies in plan and guarded initialization outputs before MCP exposure | none beyond existing guarded starter initialization outputs | starter init remains approval-gated; provenance itself is package metadata |
| `CAP-STARTER-POLICY-DISTRIBUTION-BOUNDARY-01` | Starter policy distribution hard boundary | implemented code guard | `src/jikuo/starter_policies.py`; `tests/starter_policies_tests.py`; `docs/work_orders/SPRINT_050_WO-PER-JIKUO-POLICY-CATALOG-01_self_bootstrap_policy_promotion_review.md` | prevents JIKUO's self-bootstrap `.jikuo/policies` from becoming an implicit upstream policy source and protects user policy stores from bulk overwrite during starter initialization | none in plan; guarded starter init may append reviewed package-template policies to the target project | starter manifests may reference only `pkg://jikuo/policy_templates/...`; existing user policy files and active refs must be preserved |
| `CAP-MCP-RESPONSE-PRIVACY-CLASSIFICATION-01` | MCP response privacy classification baseline | accepted contract / checklist | `docs/governance/jikuo_trust_privacy_provenance_baseline.md`; `docs/work_orders/SPRINT_050_WO-PER-JIKUO-SEC-02_mcp_response_privacy_classification_baseline.md`; `docs/work_orders/SPRINT_050_WO-PER-JIKUO-MCP-01_mcp_wrapper_mvp.md` | classifies MCP response fields as `return`, `local_only`, `redact_required`, or `redact_optional` before adapter/server implementation | none | MCP implementation must preserve the accepted classification contract |
...
| `CAP-TASK-SESSION-BINDING-EVIDENCE-01` | Task-session binding evidence | implemented by `JIKUO-LIVE-15`, hardened by `JIKUO-LIVE-21` | `python -B -m jikuo.agent_flow propose --event task_start ...` | emits structured `task_session_binding_evidence` when a task-start proposal binds an existing session, proposes guarded creation, or explicitly defers task-session creation | none | `needs_user_decision` is visible evidence, not durable creation; guarded apply is still required to write a session |
| `CAP-TASK-SESSION-RESOLUTION-01` | Governed slice task-session resolution | implemented by `JIKUO-LIVE-21` | `python -B -m jikuo.agent_flow propose --event task_start --task-session-decision defer ...` or default task-start preview | makes task-session state explicit as `existing_session_bound`, `needs_user_decision`, or `explicitly_deferred` so governed work cannot silently skip lifecycle carrier handling | none unless guarded apply is later approved | does not implement strict host enforcement; hooks must later stop agents from proceeding past unresolved decisions |
...
| `CAP-POLICY-EVOLUTION-APPLY-BINDING-01` | Proposal-to-apply binding check | implemented guarded preflight | `python -B -m jikuo.agent_flow apply --operation policy_evolution_write --proposal-ref <proposal-ref> ...` | refuses policy evolution apply when the proposal ref supplied at approval time does not match the deterministic policy evolution plan, including target operation and existing replacement policy ref for supersession | none | proposal ref, approval phrase, and technical confirmation required |
| `CAP-AGENT-FLOW-APPLY-POLICY-EVOLUTION-01` | Agent flow guarded policy evolution apply | implemented guarded apply | `python -B -m jikuo.agent_flow apply --operation policy_evolution_write --proposal-ref <proposal-ref> ... --replacement-policy-ref ... --confirm-apply --approval-phrase ...` | lets the desktop agent apply one explicitly approved policy deprecation / supersession without switching to the raw policy-store helper; supersession links to an existing replacement policy instead of creating replacement content | policy proposal snapshot; decision record; manifest lifecycle refs; existing replacement policy read proof | proposal ref, approval phrase, and technical confirmation required |
...
| `CAP-UNIVERSAL-INSTRUCTION-FILE-01` | Universal instruction file distribution | implemented by `JIKUO-INTG-01` | `src/jikuo/integrations/instruction_files.py`; `jikuo install`; `JIKUO.md`; `AGENTS.md`; `CLAUDE.md`; `.cursorrules`; `.continuerules`; `.github/copilot-instructions.md` | distributes JIKUO invocation, card-display, approval, trigger-mode onboarding, and out-of-band verification rules across clients without relying on one client hook | instruction files only | default is review-only; guarded write requires `--write`, `--confirm-install`, and `--approval-phrase` |
| `CAP-CLIENT-ONBOARDING-SETTINGS-01` | Client activation settings onboarding | implemented project settings plus instruction-plan support | `jikuo settings status|plan|apply`; `jikuo install --client codex|claude-code|cursor|vscode-copilot --trigger-mode ask|semantic|mounted`; `docs/work_orders/SPRINT_050_WO-PER-JIKUO-INTG-02_client_onboarding_settings_and_trigger_mode.md` | makes first-use JIKUO settings explicit as project-local governed state and inside client instruction files: trigger mode, effective enforcement, MCP availability, card-display contract, runtime visibility links, guarded writes, starter policies, and project-context bindings; missing activation settings now surface as a configuration obligation instead of silent fallback | `.jikuo/activation_settings.yaml` through guarded settings apply; instruction files through guarded install | `ask` is the safest default; mounted mode is a project preference until a strict pre-turn adapter is installed; missing settings report `strict_mount_status=not_configured` |
| `CAP-CONFIGURATION-REVIEW-01` | First-use configuration review | implemented no-write core aggregator plus MCP read surface | `src/jikuo/configuration_review.py`; `jikuo configure status|review`; `python -B -m jikuo.agent_flow propose --event configuration_review`; `jikuo.get_configuration_status` | gives users one readable setup status surface with meaning and next actions for activation settings, client instructions, runtime visibility, MCP availability, project context, starter policies, and guarded writes | none beyond runtime visibility projection | guarded configuration update tools and strict pre-turn adapters remain future work |
| `CAP-CONFIGURATION-SOURCE-PROVENANCE-01` | Reviewed configuration source provenance | implemented core / MCP-visible status | `src/jikuo/activation_settings.py`; `src/jikuo/agent_flow.py`; `tests/activation_settings_tests.py`; `tests/configuration_review_tests.py`; `tests/mcp_adapter_tests.py` | distinguishes implicit defaults, legacy unreviewed files, user-reviewed defaults, and user-configured values so users are asked to review key activation settings on first active use instead of inheriting hidden defaults | `.jikuo/activation_settings.yaml` only through guarded activation settings apply | no automatic writes; explicit trigger-mode overrides remain deliberate call-level overrides |
| `CAP-MCP-ACTIVATION-SETTINGS-READ-PLAN-01` | MCP activation settings read and plan | implemented no-write MCP read / plan surface | `jikuo.get_activation_settings`; `jikuo.plan_activation_settings_update`; `src/jikuo/integrations/mcp/adapter.py`; `src/jikuo/integrations/mcp/server.py` | lets desktop AI clients show current trigger-mode settings and preview activation updates without asking users to run CLI commands | none beyond runtime visibility projection | guarded MCP apply implemented separately after explicit user approval |
| `CAP-MCP-ACTIVATION-SETTINGS-APPLY-01` | MCP activation settings guarded apply | implemented guarded MCP write surface | `jikuo.apply_activation_settings_update`; `src/jikuo/integrations/mcp/adapter.py`; `src/jikuo/integrations/mcp/server.py` | lets a desktop AI client apply a reviewed semantic / mounted / ask activation setting after explicit confirmation and approval phrase | `.jikuo/activation_settings.yaml` only | refuses without `confirm_apply=true` and approval phrase; raw approval phrase is redacted |
| `CAP-MCP-CONVERSATION-ROUTER-SURFACE-01` | MCP conversation router and policy-suggestion surfaces | implemented no-write MCP surface | `jikuo.route_user_request`; `jikuo.propose_policy_suggestions`; `src/jikuo/integrations/mcp/adapter.py`; `src/jikuo/integrations/mcp/server.py` | lets desktop AI clients ask JIKUO how to route an ordinary user turn, including setup/configuration, task-start, and policy-candidate obligations, without depending only on the host Agent's guess | none beyond runtime visibility projection | strict pre-turn execution still requires a client adapter / SDK wrapper / Studio entry |
| `CAP-STRICT-MOUNTED-HARNESS-ADAPTER-CONTRACT-01` | Strict mounted harness adapter contract | accepted contract / client-proof gate | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-INTG-03_strict_mounted_harness_adapter_contract.md` | separates "mounted mode is configured" from "a host adapter actually called JIKUO before every user turn", preventing false confidence in prompt-only enforcement | none | first adapter implementation requires client proof and explicit user approval |
| `CAP-CODEX-PLUGIN-HARNESS-REVIEW-01` | Codex plugin strict-harness feasibility review | planned proof task | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-CODEX-PLUGIN-01_codex_plugin_pre_turn_harness_review.md` | determines whether Codex plugin surfaces can become a strict pre-turn JIKUO adapter or should only help with instructions, MCP setup, and runtime proof, preventing overclaiming Codex strict mounted support | none | no plugin implementation or strict-support claim until proof is accepted |
| `CAP-CLAUDE-HOOK-HARNESS-REVIEW-01` | Claude hook strict-harness feasibility review | planned proof task | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-CLAUDE-HOOK-01_claude_hook_strict_adapter_review.md` | determines whether a Claude hook-capable host can run JIKUO before every user turn, surface cards / runtime links, and fail visibly, preventing overclaiming Claude strict mounted support | none | no hook implementation or strict-support claim until proof is accepted |
| `CAP-POLICY-DEAD-ZONE-DETECTION-01` | Policy dead-zone detection | LIVE-20A implemented; history-threshold scanning remains planned | `src/jikuo/agent_flow.py`; `tests/agent_flow_tests.py`; `docs/work_orders/SPRINT_050_WO-PER-JIKUO-LIVE-20_policy_dead_zone_detection.md` | classifies active-policy evaluations with zero triggered policies as non-governance event, route follow-up required, missing/mismatched task context, condition mismatch, policy coverage gap, or unknown event, preventing users from mistaking no-op governance for healthy governance | none | no automatic policy creation; repeated-history thresholding and broader `jikuo show` surfacing remain separate work |
| `CAP-STRUCTURED-EXECUTION-EVENT-LEDGER-01` | Structured execution event ledger and analytics contract | planned design task | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-DATA-01_structured_execution_event_ledger_and_analytics_contract.md` | defines a local schema-versioned event layer for future dashboard, BI, support bundles, and audit views so those surfaces can read execution facts instead of parsing Markdown cards, latest-only snapshots, or raw chat transcripts | none in DATA-01A; later slices may append local `.jikuo/events/*.jsonl` records after approval | no remote telemetry, dashboard database, raw transcript capture, or event persistence until the schema is reviewed and accepted |
| `CAP-DOCUMENT-REGISTRY-TRANSITION-01` | Layered document registry transition | planned transition task | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-DOCREG-01_layered_document_registry.md`; `.jikuo/project_context.yaml`; `docs/README.md` | establishes a transitional authority and later registry-backed source of truth for work orders, capabilities, scenario chains, and mount sets so task planning is queryable and validated instead of hidden in a single large Markdown task map | none in DOCREG-01 planning; later DOCREG-01A may add registry files and validation tests | no task-map deletion, generated projection tooling, reverse-edge hand maintenance, runtime behavior change, or source-directory migration in the first slice |
| `CAP-STABLE-SELF-BOOTSTRAP-EXECUTION-01` | Stable JIKUO self-bootstrap execution | planned discussion task | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-SELF-BOOTSTRAP-02_stable_self_bootstrap_execution_strategy.md` | turns JIKUO development into a repeatable governed slice workflow so route/config/task-start, task-session binding, runtime-card surfacing, tests, completion review, and main-doc maintenance are not left to assistant memory | none | strategy first; implementation may span policy, CLI helper, MCP, Codex plugin, or future strict adapter |
| `CAP-SELF-BOOTSTRAP-HARNESS-WORKSPACE-BOUNDARY-01` | Self-bootstrap harness workspace boundary spike | planned proof task | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-SELF-BOOTSTRAP-03_harness_workspace_boundary_spike.md` | tests whether JIKUO self-bootstrap should open a parent harness workspace with `.jikuo/`, activation settings, runtime cards, and client instructions while the source repository is a governed child target, preventing harness state and source package state from being conflated | none | no source tree movement, GitHub layout change, or multi-root implementation before proof and user acceptance |
...
| `CAP-MCP-PREIMPLEMENTATION-API-NEUTRALITY-REVIEW-01` | MCP pre-implementation API neutrality review | accepted by `JIKUO-ARCH-03` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-ARCH-03_mcp_pre_implementation_api_neutrality_review.md` | confirms the first MCP adapter can call structured core functions instead of CLI `main()` or stdout scraping, and names remaining blockers before MCP code begins | none beyond approved task-session record | accepted review still blocks MCP implementation until revised `MCP-01` is accepted |
| `CAP-MCP-AGENT-FLOW-WRAPPER-01` | MCP wrapper around stable JIKUO atoms | Stage A accepted, Stage B1 implemented, Stage B2 externally smoke-accepted, Stage B3 externally smoke-accepted, configuration status read surface implemented, activation settings read / plan / apply implemented, and router surfaces implemented | `src/jikuo/integrations/mcp/adapter.py`; `src/jikuo/integrations/mcp/schemas.py`; `src/jikuo/integrations/mcp/server.py`; `tests/mcp_adapter_tests.py`; `tests/mcp_server_tests.py` | exposes scoped status / proposal / display-card operations, first-use configuration review, activation settings read / plan / apply, conversation routing, policy-suggestion review, plus guarded task-session evidence, policy-evolution, and policy-template activation writes to MCP clients while preserving the Desktop App Primary Operating Loop and runtime card-link visibility | no governance writes in no-write tools; activation settings apply writes `.jikuo/activation_settings.yaml`; Stage B1 writes one explicit task-session evidence item after approval; Stage B2 writes one approved policy deprecation / supersession only after proposal-ref binding; Stage B3 activates one resolved policy template after approval; card-producing tools may update `.jikuo/runtime/` | no MCP SDK import in adapter; same display contract and privacy classification as underlying atoms |
| `CAP-MCP-RUNTIME-STATUS-CARD-01` | MCP runtime status card tool | Stage A accepted by `JIKUO-MCP-01`; user-verified Codex Desktop and Claude Desktop smoke passed | `jikuo.get_runtime_status_card`; `src/jikuo/integrations/mcp/adapter.py`; `src/jikuo/integrations/mcp/server.py`; `tests/mcp_adapter_tests.py`; `tests/mcp_server_tests.py` | returns compact card Markdown, display directives, and `display_verification` so Agents have a narrow card-only path to show policy runtime status | local runtime projection may update | card markdown must be shown verbatim; local paths are transport-gated |
...
- `CAP-CLIENT-ONBOARDING-SETTINGS-01`: implemented project-level guarded activation settings persistence plus first-use trigger-mode instruction prompts; MCP activation tools remain future work
- `CAP-JIKUO-TRIGGER-MODE-01`: core and instruction-file contract is implemented; strict mounted adapters remain future work
...
- `CAP-PROGRESS-SUMMARY-BUSINESS-MEANING-POLICY-01`: active report-only policy requiring progress / todo summaries to include product or business meaning
- `CAP-STRICT-MOUNTED-HARNESS-ADAPTER-CONTRACT-01`: accepted contract for future host adapters that run JIKUO before every user turn in mounted mode
...
- root project `.jikuo/task_sessions/` now contains self-bootstrap task-session records from later LIVE slices; index refresh remains a separate guarded action.
...
- at TASKSESSION-03 verification time, root project `.jikuo/project_state.yaml` index had not been updated.
- at TASKSESSION-03 verification time, root project `.jikuo/task_sessions/` had not been created; later LIVE slices created self-bootstrap task-session records.
...
- at TASKSESSION-04 verification time, root project `.jikuo/task_sessions/` had not been created; later LIVE slices created self-bootstrap task-session records.
- at TASKSESSION-04 verification time, root project `.jikuo/project_state.yaml` index had not been updated.
...
- Does the writer require the replacement policy to already exist instead of
  creating replacement policy content?
...
- existing replacement approved policy resolution
...
- Does a successful supersession write still produce proposal snapshot,
  decision record, manifest lifecycle refs, and a lineage link to an existing
  replacement policy?
...
- `JIKUO-LIVE-07` supersedes active policies through an explicit guarded writer
  and an already existing replacement approved policy.
...
- preserve the latest todo map and keep previous todo comparison explicitly disabled until a real snapshot exists
...
- source: not mounted in v0
- status: disabled in `.jikuo/project_context.yaml` with `previous_todo_map.path: null`
- replacement: future guarded snapshot rotation capability

Delta from previous todo map:

- real previous/latest delta comparison is not available in v0
- the former same-file `previous_todo_map` fallback is removed from `.jikuo/project_context.yaml`
- future previous/latest comparison requires guarded snapshot rotation before any policy can treat it as satisfied
...
- completed `JIKUO-ARCH-03` MCP pre-implementation API neutrality review and kept MCP implementation stopped pending user discussion
- implemented `JIKUO-LIVE-16` policy runtime card priority so chat-ready/runtime cards render `policy_runtime_status` before lifecycle or task cards
- implemented `JIKUO-LIVE-17` task-session index stale hint so `jikuo show` surfaces stale/current index state without auto-refreshing project state
- implemented `JIKUO-LIVE-18` previous/latest todo snapshot posture so `previous_todo_map` is disabled in v0 instead of fake-bound to the latest taskmap
- implemented `JIKUO-LIVE-19` starter policy provenance backfill so official starter policies carry `verified_jikuo_official` provenance in both plan summaries and guarded initialization outputs
- implemented `JIKUO-SEC-02` MCP response privacy classification baseline so response fields have explicit `return`, `local_only`, `redact_required`, and `redact_optional` defaults before MCP code begins
...
- `JIKUO-ARCH-03` MCP pre-implementation API neutrality review accepted on 2026-05-15: `agent_flow.py`, `policy_store.py`, `runtime_visibility.py`, policy template planning, and starter pack planning expose structured callable surfaces; the first MCP adapter should not call CLI `main()` as its normal path.
- `JIKUO-LIVE-16` policy runtime card priority implemented on 2026-05-15: `display.card_priority_order[0]` is `policy_runtime_status`, and chat-ready/runtime Markdown renders that card first when present.
- `JIKUO-LIVE-17` task-session index stale hint implemented on 2026-05-15: `latest_task_session_refs` remains an independent guarded refresh, while `jikuo show` now reports stale/current index state and review / refresh commands.
- `JIKUO-LIVE-18` previous/latest todo snapshot posture implemented on 2026-05-15: `.jikuo/project_context.yaml previous_todo_map.path` is `null`, snapshot rotation is deferred to a future capability, and same-file fallback is guarded by regression coverage.
...
- `CAP-CLIENT-ONBOARDING-SETTINGS-01` is implemented at instruction-plan level: `jikuo install --trigger-mode ask|semantic|mounted` records first-use activation settings across Codex, Claude Code, Cursor, and VS Code Copilot instruction targets.
- `CAP-CLIENT-ONBOARDING-SETTINGS-01` now includes project-level guarded persistence: `jikuo settings plan|apply` writes `.jikuo/activation_settings.yaml` only after confirmation and approval, while `jikuo show`, `jikuo install`, and the conversation-turn router can read it.
- `CAP-JIKUO-TRIGGER-MODE-01` now has visible core behavior: semantic no-match returns `no_jikuo_action_required`; mounted no-match returns `mounted_idle_tick`.
- `CAP-CONFIGURATION-REVIEW-01` is implemented as the first-use configuration review aggregator: `jikuo configure status|review` and `agent_flow propose --event configuration_review` expose activation, instruction, runtime, MCP, project-context, starter-policy, and guarded-write setup status without durable writes.
...
5. Use `docs/governance/jikuo_policy_governance_authority.md` as the current authority before any policy-governance discussion or implementation. Its core routing rule is hint-assisted, deterministic-checked, and fallback-expanded: AI supplies semantic hints, hard signals prevent downgrade, and uncertain / mixed work expands to at least discussion plus editing policy scopes.
6. Stop before `DOCREG-01B3`: completion-review registry policy is a domain-specific consumer of the broader policy-governance model and needs explicit discussion before implementation.
7. Plan the future `POLTRIG-*` sequence from the policy-governance authority: add structured `agent_hint`, deterministic signal reporting, `discussion` / `editing` / `progress_summary` / `other` policy scopes, fallback expansion, scope-aware policy declarations, and runtime-card surfacing.
8. Accept / review `DATA-01A`: `docs/schemas/execution_events_v0_draft.md`, JSONL fixtures, and schema tests now exist without runtime writes. Next data work is deciding the DATA-01B event-emission boundary.
9. Extend `JIKUO-LIVE-20` beyond LIVE-20A: scan recent runtime history, count repeated zero-trigger classifications, and surface warnings in `jikuo show` / MCP status so users can see dead zones across time, not only on the current card.
10. Run `JIKUO-CODEX-PLUGIN-01` / future Codex hook split: Codex MCP proof is accepted and active in real work, so the remaining Codex question is whether a hook / plugin surface can provide true pre-turn mounted routing, visible failure, and runtime card surfacing.
11. Run `JIKUO-CLAUDE-HOOK-01`: Claude / Claude Code MCP proof is accepted and active in real work, so the remaining Claude question is whether a hook-capable host can call `jikuo.route_user_request` before every turn, surface cards / runtime links, and fail visibly.
12. If Codex and Claude both prove `strict_pre_turn_supported`, plan both strict mounted adapters; the decision becomes implementation order and failure semantics, not a single-winner host choice.
13. Discuss and scope `JIKUO-SELF-BOOTSTRAP-02` stable self-bootstrap execution: JIKUO development slices need a repeatable route/config/task-start/completion-review workflow before relying on assistant behavior.
14. Run `JIKUO-SELF-BOOTSTRAP-03` harness workspace boundary spike before any source-directory migration: test whether `.jikuo/`, activation settings, runtime cards, and client instructions belong in a parent harness workspace while the source repository is a governed child target.
15. Keep Cursor and VS Code + GitHub Copilot Agent Mode MCP proof as compatibility expansion using `docs/integrations/mcp_client_proof_playbook.md`; they are no longer blockers for Codex / Claude strict-mounted work.
16. Plan `JIKUO-ONBOARD-01` package-first non-engineer onboarding: install package -> run CLI setup -> configure client MCP -> verify runtime card, without making Python `import` the user entry.
17. Decide the external release license using `docs/work_orders/SPRINT_050_WO-PER-JIKUO-REL-01_external_release_license_decision.md`; current package metadata remains `Proprietary` until explicit user decision.
18. Keep the decision about whether new self-bootstrap policies enter built-in starter templates suspended until explicit user approval.
19. Plan `JIKUO-STUDIO-01` dashboard as a post-MCP product architecture slice; it is deferred from MCP-01 scope and current Stage B work relies on card markdown plus runtime card links instead. Mount `docs/insights/INSIGHT-2026-05-16-studio-dashboard-frontend-architecture.md` and `JIKUO-DATA-01` before that slice.
20. Keep per-client hooks/packs and Agent SDK / platform adapters as planned post-MCP work, not current MCP blockers.
21. Defer OS notifications, rollback, broader conditions, UI beyond Studio, Plugin, and gates unless explicitly promoted by user approval.
...
- MCP client configuration examples live at `docs/integrations/mcp_client_configuration_examples.md`, and the user-facing proof workflow lives at `docs/integrations/mcp_client_proof_playbook.md`; Codex / Claude MCP proof is accepted for current private-preview use, while Cursor and VS Code + GitHub Copilot Agent Mode remain compatibility-expansion proof targets. Private GitHub proof should clone from `https://github.com/jikuo-studio/jikuo.git` while the repo remains private.
- Stage A accepted: user verified real desktop-client smoke in Codex Desktop and Claude Desktop on 2026-05-15, and both Codex / Claude are now treated as active private-preview MCP clients; local client/test temporary directories are expected under ignored paths such as `.claude/` and `tmp/`
...
- Stage B2 implemented and externally smoke-accepted on 2026-05-16: `jikuo.apply_policy_evolution_write` refuses without confirmation / approval phrase / proposal ref, applies one approved policy deprecation / supersession only when the proposal ref matches, does not write task-session evidence, writes fixture runtime last-card output, and does not return the raw approval phrase
- Stage B3 implemented and externally smoke-accepted on 2026-05-16: `jikuo.apply_policy_template_activation` refuses without confirmation / approval phrase, activates one resolved policy template only after approval, writes proposal snapshot / approved policy / decision record / manifest, does not write task-session evidence, writes fixture runtime last-card output, does not return the raw approval phrase, and showed no GUI MCP client tool-list cache issue
- MCP MVP body release smoke passed on 2026-05-16: official SDK `ClientSession` listed all 11 tools and `jikuo.get_runtime_status_card` returned card Markdown matching `.jikuo/runtime/last_card.md` in a temporary fixture project
- Post-MVP configuration / activation settings MCP surface implemented after `JIKUO-INIT-01`: `jikuo.get_configuration_status` returns `jikuo.configuration_review.v0`; `jikuo.get_activation_settings` returns `jikuo.activation_settings_status.v0`; `jikuo.plan_activation_settings_update` returns `jikuo.activation_settings_plan.v0`; guarded `jikuo.apply_activation_settings_update` writes only `.jikuo/activation_settings.yaml` after confirmation and approval phrase
- Post-MVP router MCP surface implemented after configuration acceptance: `jikuo.route_user_request` returns `jikuo.conversation_turn_router.v0` plus MCP-specific follow-up tools; `jikuo.propose_policy_suggestions` returns `jikuo.proactive_policy_suggestion_review.v0`; both are no-write except runtime visibility projection; current tool discovery should list 17 tools
- Current strict-mounted blocker: Codex / Claude MCP availability is no longer the main uncertainty; NarrativeSystem usage shows the harder problem is zero-trigger policy dead zones plus the lack of a proven pre-turn host hook, so LIVE-20 history scanning and Codex / Claude hook feasibility now lead the queue. LIVE-20A per-card classification is implemented.
- Codex and Claude strict-hook tracks are parallel candidates; if both support stable pre-turn execution, implement both adapters after user approval rather than forcing a single-client choice.
- Self-bootstrap harness workspace boundary insight promoted on 2026-05-17: `JIKUO-SELF-BOOTSTRAP-03` must test a parent harness workspace with child source repository before any source-directory migration or multi-root model is claimed.
...
- Stage A release gate: at least two available MCP-compatible desktop clients are tested, or unavailable client access is explicitly recorded; current gate is accepted by user-verified Codex Desktop and Claude Desktop smoke
- Stage B1 implemented after Stage A acceptance: guarded task-session evidence apply
- Stage B2 implemented after user approval to continue MCP Stage B: guarded policy evolution apply with proposal-ref binding
- Stage B3 implemented after explicit user approval: guarded policy-template activation apply
...
- require: no-write MCP proposal calls return `chat_ready_markdown` and preserve `policy_runtime_status` as first-screen card output when present
...
- `JIKUO-SEC-02` response privacy classification is accepted; MCP implementation must preserve its field-level return / local-only / redaction contract
...
- no MCP implementation before `JIKUO-ARCH-03` API neutrality review is accepted or explicitly deferred
- no MCP implementation before `JIKUO-SDK-01` Agent SDK extension posture is accepted or explicitly deferred
- starter policy provenance exposure blocker is resolved by `JIKUO-LIVE-19`; future MCP starter-policy tools must still refuse missing-provenance records
