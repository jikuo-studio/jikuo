...
- `JIKUO-MCP-01`: MCP wrapper MVP work order, Stage A accepted as user-verified; `src/jikuo/integrations/mcp/adapter.py`, `schemas.py`, and `server.py` expose 8 no-write / card / proposal tools, SDK client smoke passed, and Codex Desktop / Claude Desktop real-client smoke is accepted while Stage B guarded writes remain blocked
...
| `CAP-MCP-AGENT-FLOW-WRAPPER-01` | MCP wrapper around stable JIKUO atoms | Stage A accepted by `JIKUO-MCP-01`; server wrapper and two-client smoke passed | `src/jikuo/integrations/mcp/adapter.py`; `src/jikuo/integrations/mcp/schemas.py`; `src/jikuo/integrations/mcp/server.py`; `tests/mcp_adapter_tests.py`; `tests/mcp_server_tests.py` | exposes scoped status / proposal / display-card operations to MCP clients while preserving the Desktop App Primary Operating Loop and out-of-band runtime visibility | no governance writes in Stage A; card-producing tools may update `.jikuo/runtime/` | no MCP SDK import in adapter; same display contract and privacy classification as underlying atoms |
