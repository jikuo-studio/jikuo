...
| `CAP-CONVERSATION-TURN-ROUTER-01` | Conversation-turn router | implemented core no-write router | `python -B -m jikuo.agent_flow propose --event conversation_turn --user-phrase ... --trigger-mode semantic|mounted`; future `jikuo.route_user_request` MCP tool | classifies ordinary user turns into JIKUO obligations, including task start, completion, policy suggestion review, insight / follow-up routing, clarification, semantic no-op, and mounted idle ticks, instead of relying only on task-specific events | none beyond runtime visibility projection | MCP / mounted adapters remain future work |
...
2. Add MCP router surfaces after the configuration / activation settings paths are accepted: `jikuo.route_user_request` and `jikuo.propose_policy_suggestions` belong in follow-on MCP slices, not the already accepted MVP body.
3. Plan strict mounted harness entry after the router exists, using Agent SDK wrappers, local proxy / Studio entry, or client hooks as adapters rather than kernel logic.
...
- MCP client configuration examples live at `docs/integrations/mcp_client_configuration_examples.md` for Claude Code, Claude Desktop, Cursor, and Codex-style stdio setup notes; they now describe the current 15-tool surface, including configuration / activation settings read-plan / guarded-apply tools plus B1 / B2 / B3 guarded-write smoke checks
