...
- `JIKUO-SEC-02`: MCP response privacy classification baseline, implemented on 2026-05-15 and ready for user review; SEC-01 now defines field-level `return` / `local_only` / `redact_required` / `redact_optional` classifications and MCP-01 has a startup checklist
...
- `JIKUO-ARCH-03`: MCP pre-implementation API neutrality review, completed and accepted on 2026-05-15; confirms the first MCP adapter can wrap structured core functions without calling CLI `main()` or adding MCP-specific kernel behavior, while keeping `SEC-02` and revised `MCP-01` acceptance visible as blockers
...
| `CAP-MCP-RESPONSE-PRIVACY-CLASSIFICATION-01` | MCP response privacy classification baseline | implemented contract / checklist | `docs/governance/jikuo_trust_privacy_provenance_baseline.md`; `docs/work_orders/SPRINT_050_WO-PER-JIKUO-SEC-02_mcp_response_privacy_classification_baseline.md`; `docs/work_orders/SPRINT_050_WO-PER-JIKUO-MCP-01_mcp_wrapper_mvp.md` | classifies MCP response fields as `return`, `local_only`, `redact_required`, or `redact_optional` before adapter/server implementation | none | user acceptance required before MCP implementation begins |
...
| `CAP-MCP-PREIMPLEMENTATION-API-NEUTRALITY-REVIEW-01` | MCP pre-implementation API neutrality review | accepted by `JIKUO-ARCH-03` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-ARCH-03_mcp_pre_implementation_api_neutrality_review.md` | confirms the first MCP adapter can call structured core functions instead of CLI `main()` or stdout scraping, and names remaining blockers before MCP code begins | none beyond approved task-session record | accepted review still blocks MCP implementation until `SEC-02` and revised `MCP-01` are accepted |
...
- `JIKUO-SEC-02` MCP response privacy classification baseline implemented on 2026-05-15: SEC-01 defines field-level response privacy classes and MCP-01 lists them in startup checklist / test requirements.

Open items:

1. Review / accept `JIKUO-SEC-02` field-level MCP response privacy classification.
2. Review / accept revised `JIKUO-MCP-01` visibility and integration-neutral scope: structured tools, card-only tools, `jikuo.get_runtime_status`, `jikuo.get_runtime_status_card`, `jikuo.get_display_card`, display directives, runtime snapshot refs, startup checklist, UX acceptance standard, and implementation under `src/jikuo/integrations/mcp/`.
...
4. Return to `JIKUO-MCP-01` implementation only after the above pre-MCP items are accepted or explicitly deferred; stop for user discussion before code implementation.
...
- no MCP implementation before `JIKUO-SEC-02` response privacy classification is accepted
