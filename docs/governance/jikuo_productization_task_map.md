...
- `JIKUO-MCP-01`: MCP wrapper MVP work order, Stage A accepted as user-verified, Stage B1 implemented after explicit user approval, and Stage B2 implemented plus externally smoke-accepted on 2026-05-16; `src/jikuo/integrations/mcp/adapter.py`, `schemas.py`, and `server.py` expose 8 no-write / card / proposal tools plus `jikuo.apply_task_session_evidence_update` and `jikuo.apply_policy_evolution_write`, while template activation MCP writes remain blocked
...
| `CAP-MCP-AGENT-FLOW-WRAPPER-01` | MCP wrapper around stable JIKUO atoms | Stage A accepted, Stage B1 implemented, and Stage B2 externally smoke-accepted by `JIKUO-MCP-01`; server wrapper and two-client smoke passed for Stage A | `src/jikuo/integrations/mcp/adapter.py`; `src/jikuo/integrations/mcp/schemas.py`; `src/jikuo/integrations/mcp/server.py`; `tests/mcp_adapter_tests.py`; `tests/mcp_server_tests.py` | exposes scoped status / proposal / display-card operations plus guarded task-session evidence and policy-evolution writes to MCP clients while preserving the Desktop App Primary Operating Loop and runtime card-link visibility | no governance writes in Stage A; Stage B1 writes one explicit task-session evidence item after approval; Stage B2 writes one approved policy deprecation / supersession only after proposal-ref binding; card-producing tools may update `.jikuo/runtime/` | no MCP SDK import in adapter; same display contract and privacy classification as underlying atoms |
...
5. Keep per-client hooks/packs and Agent SDK / platform adapters as planned post-MCP work, not current MCP blockers.
6. Defer OS notifications, rollback, broader conditions, UI beyond Studio, Plugin, and gates unless explicitly promoted by user approval.
...
- Stage B3 remains blocked: guarded policy-template activation apply
