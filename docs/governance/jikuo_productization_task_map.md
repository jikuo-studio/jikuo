...
| `first_use_configuration_review` | A user first enables JIKUO and needs to understand current setup state and business meaning before governed work continues | user asks to initialize / configure JIKUO -> no-write configuration review aggregates activation, instructions, runtime, MCP, project context, starter policies, and guarded writes -> agent-flow card explains status and next actions -> later MCP / Studio surfaces reuse the same review object | `CAP-CONFIGURATION-REVIEW-01`; `CAP-CLIENT-ONBOARDING-SETTINGS-01`; `CAP-RUNTIME-SHOW-CLI-01`; `CAP-MCP-AGENT-FLOW-WRAPPER-01`; `CAP-CONVERSATION-TURN-ROUTER-01` | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-INIT-01_first_use_configuration_review.md` |
...
| `CAP-MCP-AGENT-FLOW-WRAPPER-01` | MCP wrapper around stable JIKUO atoms | Stage A accepted, Stage B1 implemented, Stage B2 externally smoke-accepted, Stage B3 externally smoke-accepted, and configuration status read surface implemented | `src/jikuo/integrations/mcp/adapter.py`; `src/jikuo/integrations/mcp/schemas.py`; `src/jikuo/integrations/mcp/server.py`; `tests/mcp_adapter_tests.py`; `tests/mcp_server_tests.py` | exposes scoped status / proposal / display-card operations, first-use configuration review, plus guarded task-session evidence, policy-evolution, and policy-template activation writes to MCP clients while preserving the Desktop App Primary Operating Loop and runtime card-link visibility | no governance writes in no-write tools; Stage B1 writes one explicit task-session evidence item after approval; Stage B2 writes one approved policy deprecation / supersession only after proposal-ref binding; Stage B3 activates one resolved policy template after approval; card-producing tools may update `.jikuo/runtime/` | no MCP SDK import in adapter; same display contract and privacy classification as underlying atoms |
...
2. Add MCP configuration update / activation settings tools and router surfaces after the read-only configuration status path is accepted: `jikuo.get_activation_settings`, `jikuo.plan_activation_settings_update`, `jikuo.apply_activation_settings_update`, `jikuo.route_user_request`, and `jikuo.propose_policy_suggestions` belong in a follow-on MCP slice, not the already accepted MVP body.
...
- MCP client configuration examples live at `docs/integrations/mcp_client_configuration_examples.md` for Claude Code, Claude Desktop, Cursor, and Codex-style stdio setup notes; they now describe the current 12-tool surface, including configuration status plus B1 / B2 / B3 guarded-write smoke checks
...
- Post-MVP configuration status MCP read surface implemented after `JIKUO-INIT-01`: `jikuo.get_configuration_status` returns `jikuo.configuration_review.v0`, card markdown, display verification, and runtime visibility without governance writes; current tool discovery should list 12 tools
