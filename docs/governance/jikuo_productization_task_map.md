...
- `JIKUO-MCP-01`: MCP wrapper MVP work order, Stage A accepted as user-verified and Stage B1 implemented after explicit user approval; `src/jikuo/integrations/mcp/adapter.py`, `schemas.py`, and `server.py` expose 8 no-write / card / proposal tools plus `jikuo.apply_task_session_evidence_update`, while policy evolution and template activation MCP writes remain blocked
...
| `CAP-MCP-AGENT-FLOW-WRAPPER-01` | MCP wrapper around stable JIKUO atoms | Stage A accepted and Stage B1 implemented by `JIKUO-MCP-01`; server wrapper and two-client smoke passed | `src/jikuo/integrations/mcp/adapter.py`; `src/jikuo/integrations/mcp/schemas.py`; `src/jikuo/integrations/mcp/server.py`; `tests/mcp_adapter_tests.py`; `tests/mcp_server_tests.py` | exposes scoped status / proposal / display-card operations plus one guarded task-session evidence write to MCP clients while preserving the Desktop App Primary Operating Loop and out-of-band runtime visibility | no governance writes in Stage A; Stage B1 writes one explicit task-session evidence item after approval; card-producing tools may update `.jikuo/runtime/` | no MCP SDK import in adapter; same display contract and privacy classification as underlying atoms |
...
2. Keep Stage B2 / Stage B3 guarded write tools blocked until the user explicitly accepts policy evolution or policy-template activation through MCP.
...
- Stage B1 implemented and SDK-smoked: `jikuo.apply_task_session_evidence_update` is exposed as the only guarded-write MCP tool; it refuses without confirmation / approval phrase and writes only one explicit task-session evidence item on success; official SDK `ClientSession` smoke listed 9 tools and confirmed Stage B2 / B3 are not exposed
...
- Stage B2 remains blocked: guarded policy evolution apply with proposal-ref binding
