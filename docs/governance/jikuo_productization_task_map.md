...
| `CAP-MCP-ACTIVATION-SETTINGS-READ-PLAN-01` | MCP activation settings read and plan | implemented no-write MCP read / plan surface | `jikuo.get_activation_settings`; `jikuo.plan_activation_settings_update`; `src/jikuo/integrations/mcp/adapter.py`; `src/jikuo/integrations/mcp/server.py` | lets desktop AI clients show current trigger-mode settings and preview activation updates without asking users to run CLI commands | none beyond runtime visibility projection | guarded MCP apply remains unimplemented until explicit user approval |
...
| `CAP-MCP-AGENT-FLOW-WRAPPER-01` | MCP wrapper around stable JIKUO atoms | Stage A accepted, Stage B1 implemented, Stage B2 externally smoke-accepted, Stage B3 externally smoke-accepted, configuration status read surface implemented, and activation settings read / plan surface implemented | `src/jikuo/integrations/mcp/adapter.py`; `src/jikuo/integrations/mcp/schemas.py`; `src/jikuo/integrations/mcp/server.py`; `tests/mcp_adapter_tests.py`; `tests/mcp_server_tests.py` | exposes scoped status / proposal / display-card operations, first-use configuration review, activation settings read / plan, plus guarded task-session evidence, policy-evolution, and policy-template activation writes to MCP clients while preserving the Desktop App Primary Operating Loop and runtime card-link visibility | no governance writes in no-write tools; Stage B1 writes one explicit task-session evidence item after approval; Stage B2 writes one approved policy deprecation / supersession only after proposal-ref binding; Stage B3 activates one resolved policy template after approval; card-producing tools may update `.jikuo/runtime/` | no MCP SDK import in adapter; same display contract and privacy classification as underlying atoms |
...
2. Add MCP guarded configuration apply and router surfaces after the read-only configuration / activation settings paths are accepted: `jikuo.apply_activation_settings_update`, `jikuo.route_user_request`, and `jikuo.propose_policy_suggestions` belong in follow-on MCP slices, not the already accepted MVP body.
...
- MCP client configuration examples live at `docs/integrations/mcp_client_configuration_examples.md` for Claude Code, Claude Desktop, Cursor, and Codex-style stdio setup notes; they now describe the current 14-tool surface, including configuration / activation settings read-plan tools plus B1 / B2 / B3 guarded-write smoke checks
