...
| `CAP-JIKUO-TRIGGER-MODE-01` | JIKUO trigger-mode contract | implemented core / instruction contract | `python -B -m jikuo.agent_flow propose --event conversation_turn --user-phrase ... --trigger-mode semantic|mounted`; `jikuo install --trigger-mode ask|semantic|mounted` | distinguishes natural-language semantic triggering from mounted harness triggering, so users can choose lightweight invocation or strict pre-turn harness execution and see that choice in client onboarding instructions | none beyond instruction-file plans / guarded instruction writes | strict mounted execution still requires a host adapter; no client should silently default to mounted without user choice |
...
| `CAP-CLIENT-ONBOARDING-SETTINGS-01` | Client activation settings onboarding | implemented instruction-plan support | `jikuo install --client codex|claude-code|cursor|vscode-copilot --trigger-mode ask|semantic|mounted`; `docs/work_orders/SPRINT_050_WO-PER-JIKUO-INTG-02_client_onboarding_settings_and_trigger_mode.md` | makes first-use JIKUO settings explicit inside client instruction files: trigger mode, MCP availability, card-display contract, runtime visibility links, guarded writes, starter policies, and project-context bindings | instruction files only; future guarded project settings may write `.jikuo/project_context.yaml` or a dedicated settings file | `ask` is the safest default; mounted mode in instruction files is a request until a strict pre-turn adapter is installed |
...
- `CAP-CLIENT-ONBOARDING-SETTINGS-01`: implemented instruction-plan support for first-use trigger-mode and activation setting prompts; guarded project-level settings persistence remains future work
...
3. Add guarded project-level persistence for activation settings so `ask` / `semantic` / `mounted` and related onboarding choices do not live only inside generated instruction files.
4. Add MCP router surfaces after the core router is accepted: `jikuo.route_user_request` and `jikuo.propose_policy_suggestions` belong in a follow-on MCP slice, not the already accepted MVP body.
5. Plan strict mounted harness entry after the router exists, using Agent SDK wrappers, local proxy / Studio entry, or client hooks as adapters rather than kernel logic.
6. Decide the external release license using `docs/work_orders/SPRINT_050_WO-PER-JIKUO-REL-01_external_release_license_decision.md`; current package metadata remains `Proprietary` until explicit user decision.
7. Keep the decision about whether new self-bootstrap policies enter built-in starter templates suspended until explicit user approval.
8. Plan `JIKUO-STUDIO-01` dashboard as a post-MCP product architecture slice; it is deferred from MCP-01 scope and current Stage B work relies on card markdown plus runtime card links instead. Mount `docs/insights/INSIGHT-2026-05-16-studio-dashboard-frontend-architecture.md` before that slice.
9. Keep per-client hooks/packs and Agent SDK / platform adapters as planned post-MCP work, not current MCP blockers.
10. Defer OS notifications, rollback, broader conditions, UI beyond Studio, Plugin, and gates unless explicitly promoted by user approval.
