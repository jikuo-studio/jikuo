...
- `JIKUO-MCP-01`: MCP wrapper MVP body implemented and release-smoked; Stage A accepted as user-verified, Stage B1 implemented after explicit user approval, Stage B2 implemented plus externally smoke-accepted on 2026-05-16, and Stage B3 implemented plus externally smoke-accepted on 2026-05-16; `src/jikuo/integrations/mcp/adapter.py`, `schemas.py`, and `server.py` expose 8 no-write / card / proposal tools plus `jikuo.apply_task_session_evidence_update`, `jikuo.apply_policy_evolution_write`, and `jikuo.apply_policy_template_activation`
...
- `JIKUO-INIT-01`: first-use configuration review, implemented as a no-write core aggregator and agent-flow card; MCP configuration tools and strict client adapters remain planned
...
| `CAP-CONFIGURATION-REVIEW-01` | First-use configuration review | implemented no-write core aggregator | `src/jikuo/configuration_review.py`; `jikuo configure status|review`; `python -B -m jikuo.agent_flow propose --event configuration_review` | gives users one readable setup status surface with meaning and next actions for activation settings, client instructions, runtime visibility, MCP availability, project context, starter policies, and guarded writes | none | MCP configuration tools and strict pre-turn adapters remain future work |
...
| `CAP-MCP-AGENT-FLOW-WRAPPER-01` | MCP wrapper around stable JIKUO atoms | Stage A accepted, Stage B1 implemented, Stage B2 externally smoke-accepted, and Stage B3 externally smoke-accepted by `JIKUO-MCP-01`; server wrapper and two-client smoke passed for Stage A | `src/jikuo/integrations/mcp/adapter.py`; `src/jikuo/integrations/mcp/schemas.py`; `src/jikuo/integrations/mcp/server.py`; `tests/mcp_adapter_tests.py`; `tests/mcp_server_tests.py` | exposes scoped status / proposal / display-card operations plus guarded task-session evidence, policy-evolution, and policy-template activation writes to MCP clients while preserving the Desktop App Primary Operating Loop and runtime card-link visibility | no governance writes in Stage A; Stage B1 writes one explicit task-session evidence item after approval; Stage B2 writes one approved policy deprecation / supersession only after proposal-ref binding; Stage B3 activates one resolved policy template after approval; card-producing tools may update `.jikuo/runtime/` | no MCP SDK import in adapter; same display contract and privacy classification as underlying atoms |
...
- MCP client configuration examples live at `docs/integrations/mcp_client_configuration_examples.md` for Claude Code, Claude Desktop, Cursor, and Codex-style stdio setup notes; they now describe the current 11-tool MVP surface plus B1 / B2 / B3 guarded-write smoke checks
