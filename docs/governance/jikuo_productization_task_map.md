...
- `JIKUO-MCP-01`: MCP wrapper MVP work order, Stage A adapter boundary in progress; `src/jikuo/integrations/mcp/adapter.py` and `schemas.py` now wrap stable `agent_flow.py` / `policy_store.py` atoms without an MCP SDK dependency, while MCP `server.py` and real-client release gates remain pending
...
| `CAP-MCP-AGENT-FLOW-WRAPPER-01` | MCP wrapper around stable JIKUO atoms | Stage A SDK-free adapter implemented by `JIKUO-MCP-01`; server wrapper pending | `src/jikuo/integrations/mcp/adapter.py`; `src/jikuo/integrations/mcp/schemas.py`; `tests/mcp_adapter_tests.py`; `src/jikuo/integrations/mcp/server.py` planned | exposes scoped status / proposal / display-card operations to a future MCP server while preserving the Desktop App Primary Operating Loop and out-of-band runtime visibility | no governance writes in Stage A; card-producing tools may update `.jikuo/runtime/` | no MCP SDK import in adapter; same display contract and privacy classification as underlying atoms |
| `CAP-MCP-RUNTIME-STATUS-CARD-01` | MCP runtime status card tool | Stage A adapter implemented by `JIKUO-MCP-01`; real MCP client smoke pending | `jikuo.get_runtime_status_card`; `src/jikuo/integrations/mcp/adapter.py`; `tests/mcp_adapter_tests.py` | returns compact card Markdown, display directives, and `display_verification` so Agents have a narrow card-only path to show policy runtime status | local runtime projection may update | card markdown must be shown verbatim; local paths are transport-gated |
