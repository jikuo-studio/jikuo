...
- `JIKUO-LIVE-20`: policy dead-zone detection, planned on 2026-05-17; detects and explains repeated active-policy evaluations with zero triggered policies so users do not mistake governance no-ops for healthy coverage
...
| `CAP-POLICY-DEAD-ZONE-DETECTION-01` | Policy dead-zone detection | planned discussion task | `docs/work_orders/SPRINT_050_WO-PER-JIKUO-LIVE-20_policy_dead_zone_detection.md` | detects repeated active-policy evaluations with zero triggered policies and explains whether the cause is wrong event, missing task context, missing activation, or coverage gaps, preventing users from mistaking no-op governance for healthy governance | none | detection strategy must be discussed before implementation; no automatic policy creation |
...
3. Run `JIKUO-CODEX-PLUGIN-01` after Codex proof: verify whether Codex plugin surfaces can provide a true pre-turn harness or only instruction / MCP setup assistance before claiming Codex strict mounted support.
4. Discuss and scope `JIKUO-LIVE-20` policy dead-zone detection before implementation: repeated active-policy cards with zero triggered policies must be classified and surfaced instead of silently implying healthy governance.
5. Discuss and scope `JIKUO-SELF-BOOTSTRAP-02` stable self-bootstrap execution: JIKUO development slices need a repeatable route/config/task-start/completion-review workflow before relying on assistant behavior.
6. Plan `JIKUO-ONBOARD-01` package-first non-engineer onboarding: install package -> run CLI setup -> configure client MCP -> verify runtime card, without making Python `import` the user entry.
7. Decide the external release license using `docs/work_orders/SPRINT_050_WO-PER-JIKUO-REL-01_external_release_license_decision.md`; current package metadata remains `Proprietary` until explicit user decision.
...
9. Plan `JIKUO-STUDIO-01` dashboard as a post-MCP product architecture slice; it is deferred from MCP-01 scope and current Stage B work relies on card markdown plus runtime card links instead. Mount `docs/insights/INSIGHT-2026-05-16-studio-dashboard-frontend-architecture.md` before that slice.
10. Keep per-client hooks/packs and Agent SDK / platform adapters as planned post-MCP work, not current MCP blockers.
11. Defer OS notifications, rollback, broader conditions, UI beyond Studio, Plugin, and gates unless explicitly promoted by user approval.
