...
| `jikuo.propose_policy_evolution_plan` | `CAP-POLICY-EVOLUTION-PLAN-PROPOSE-01` | no governance write; may update `.jikuo/runtime/` | return refinement / deprecation / supersession plan data; supersession plans reference an existing replacement policy ref, while refinement plans may preview trigger-profile changes |
...
| `jikuo.apply_policy_evolution_write` | `CAP-POLICY-EVOLUTION-APPLY-BINDING-01` and `CAP-AGENT-FLOW-APPLY-POLICY-EVOLUTION-01` | guarded write | apply one approved policy deprecation / supersession only when proposal ref, operation, existing replacement policy ref when applicable, confirmation, and approval phrase match |
...
| Evolve an existing rule | Agent calls `jikuo.propose_policy_evolution_plan`; user reviews refinement, deprecation, or supersession lineage to an existing replacement policy through a display card | `CAP-POLICY-EVOLUTION-PLAN-PROPOSE-01`, `CAP-RUNTIME-VISIBILITY-CHANNEL-01` | no governance write; runtime snapshot may update |
...
- policy evolution apply refuses mismatched proposal refs, including
  mismatches caused by a different target operation or existing replacement
  policy ref
